    ...
    try:
        ...
        for i, img_path in enumerate(test_images, 1):
            ...
            if possession_triangles:
                best_possession = detector.select_best_single_triangles(
                    possession_triangles, "possession"
                )
                if best_possession:
                    print(
                        f"   Best: {best_possession['direction']} (conf: {best_possession['confidence']:.3f})"
                    )
            ...
            if territory_triangles:
                best_territory = detector.select_best_single_triangles(
                    territory_triangles, "territory"
                )
                if best_territory:
                    print(
                        f"   Best: {best_territory['direction']} (conf: {best_territory['confidence']:.3f})"
                    )
                    successful_detections += 1
