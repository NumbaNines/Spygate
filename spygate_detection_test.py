        ...
        for result in ocr_results:
            ...
            for key, value in text_info.items():
                if value is not None:
                    text = f"{key}: {value}"
                    cv2.putText(
                        img, text, (x1, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1
                    )
                    y_offset -= 15
...
def comprehensive_detection_test():
    ...
    try:
        ...
        print(f"   ✅ Hardware Tier: {tier.name}")
        print(
            f"   ✅ Has EasyOCR Reader: {hasattr(ocr_engine, 'reader') and ocr_engine.reader is not None}"
        )
    ...
    for img_num, image_path in enumerate(images, 1):
        ...
        try:
            ...
            print(
                f"      YOLO Performance: {yolo_detections/yolo_time:.1f} detections/sec"
                if yolo_time > 0
                else "      YOLO Performance: N/A"
            )
            print(
                f"      OCR Performance: {len(ocr_results)/ocr_time:.1f} regions/sec"
                if ocr_time > 0 and ocr_results
                else "      OCR Performance: N/A"
            )
            ...
            indicators = [
                d for d in detection_details if "indicator" in d["class"] or "screen" in d["class"]
            ]
            ...
            image_results["performance"] = {"fps": fps_estimate, "rating": rating}
    ...
    stats["average_time_per_image"] = (
        stats["total_time"] / stats["total_images"] if stats["total_images"] > 0 else 0
    )
    stats["average_fps"] = (
