...
class TriangleOrientationDetector:
    ...
    def analyze_possession_triangle(
        self, contour: np.ndarray, frame: Optional[np.ndarray] = None
    ) -> TriangleOrientation:
        """
        ...
        try:
            ...
            if not validation_result[0]:
                return self._create_invalid_result(
                    contour, TriangleType.POSSESSION, reason=validation_result[1]
                )
            ...
            if aspect < self.POSSESSION_MIN_ASPECT:  # Must be at least 1.2 times wider than tall
                return self._create_invalid_result(
                    contour, TriangleType.POSSESSION, center, "Aspect ratio too low for arrow"
                )
            ...
            for point in contour:
                ...
                dist = np.abs(
                    np.cross(left_right_line[1] - left_right_line[0], point - left_right_line[0])
                ) / np.linalg.norm(left_right_line[1] - left_right_line[0])
            ...
            confidence = self._calculate_enhanced_confidence(
                contour, aspect, TriangleType.POSSESSION
            )

            # Save debug visualization if enabled
            if self.debug_output_dir and frame is not None:
                self._save_debug_visualization(
                    frame, contour, direction, confidence, TriangleType.POSSESSION
                )
            ...
            return self._create_invalid_result(
                contour, TriangleType.POSSESSION, reason=f"Exception: {e}"
            )

    def analyze_territory_triangle(
        self, contour: np.ndarray, frame: Optional[np.ndarray] = None
    ) -> TriangleOrientation:
        """
        ...
        try:
            ...
            if not validation_result[0]:
                return self._create_invalid_result(
                    contour, TriangleType.TERRITORY, reason=validation_result[1]
                )
            ...
            aspect = w / h if h != 0 else 0
            if not (
                self.TERRITORY_MIN_ASPECT <= aspect <= self.TERRITORY_MAX_ASPECT
            ):  # Should be between 0.8 and 1.2
                return self._create_invalid_result(
                    contour,
                    TriangleType.TERRITORY,
                    center,
                    "Aspect ratio not suitable for territory triangle",
                )
            ...
            for point in contour:
                ...
                dist = np.abs(
                    np.cross(top_bottom_line[1] - top_bottom_line[0], point - top_bottom_line[0])
                ) / np.linalg.norm(top_bottom_line[1] - top_bottom_line[0])
                distances.append((dist, point))
            ...
            confidence = self._calculate_enhanced_confidence(
                contour, aspect, TriangleType.TERRITORY
            )

            # Save debug visualization if enabled
            if self.debug_output_dir and frame is not None:
                self._save_debug_visualization(
                    frame, contour, direction, confidence, TriangleType.TERRITORY
                )
            ...
            return self._create_invalid_result(
                contour, TriangleType.TERRITORY, reason=f"Exception: {e}"
            )

    def _validate_enhanced_triangle(
        self, contour: np.ndarray, triangle_type: TriangleType
    ) -> tuple[bool, str]:
        """
    ...
    def _validate_basic_triangle(self, contour: np.ndarray) -> bool:
        ...
        try:
            ...
            if not (self.MIN_ASPECT <= aspect <= self.MAX_ASPECT):
                logger.debug(
                    f"Aspect ratio {aspect} outside valid range [{self.MIN_ASPECT}, {self.MAX_ASPECT}]"
                )
                return False
    ...
    def _calculate_enhanced_confidence(
        self, contour: np.ndarray, aspect: float, triangle_type: TriangleType
    ) -> float:
        """
        ...
        try:
            ...
            vertices = self._find_main_vertices(contour)
            vertex_confidence = max(
                0.0, 1.0 - (len(vertices) - 3) / 3
            )  # Penalty for extra vertices
