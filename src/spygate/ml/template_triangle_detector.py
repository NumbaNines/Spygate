...
class TemplateTriangleDetector:
    ...
    def detect_triangles_in_roi(self, roi_img: np.ndarray, triangle_type_str: str) -> List[TemplateMatch]:
        """
        ...
        triangle_type = TriangleType.POSSESSION if triangle_type_str == "possession" else TriangleType.TERRITORY
    ...
    def select_best_single_triangles(self, matches: List[TemplateMatch], triangle_type_str: str) -> Optional[Dict]:
        """
    ...
    def _select_best_triangle(self, matches: List[TemplateMatch], triangle_type: str) -> Optional[TemplateMatch]:
        ...
        for match in matches:
            ...
            template_quality_score = self._calculate_template_quality_score(match.template_name) * 0.25  # 25% weight
            size_score = self._calculate_smart_size_score(match, avg_area) * 0.15  # 15% weight
            position_score = self._calculate_position_score(match, triangle_type) * 0.10  # 10% weight
            ...
            total_score = (confidence_score + template_quality_score + size_score + 
                          position_score + scale_score + aspect_score)
    ...
    def calculate_overlap(self, box1: Tuple[int, int, int, int], box2: Tuple[int, int, int, int]) -> float:
    ...
    def create_triangle_template(self, name: str, direction: Direction, triangle_type: TriangleType):
    ...
    def save_template(self, name: str, template: np.ndarray, direction: Direction, triangle_type: TriangleType):
...
class YOLOIntegratedTriangleDetector:
    ...
    def detect_triangles_in_yolo_regions(self, frame: np.ndarray) -> List[TemplateMatch]:
        ...
        print(f"🎯 Triangle Detection Summary: {len(all_matches)} raw → {len(nms_matches)} after NMS → {len(final_matches)} final")
    ...
    def match_templates_in_roi(self, roi_img: np.ndarray, triangle_type: TriangleType,
                              offset: Tuple[int, int]) -> List[TemplateMatch]:
        """
    ...
    def _select_best_triangle(self, matches: List[TemplateMatch], triangle_type: str) -> Optional[TemplateMatch]:
        ...
        for match in matches:
            ...
            composite_score = (
                confidence_score * 0.35 +        # 35% confidence (most important)
                template_quality_score * 0.25 +  # 25% template quality
                size_score * 0.15 +              # 15% smart size
                position_score * 0.10 +          # 10% position
                scale_score * 0.10 +             # 10% scale factor
                aspect_ratio_score * 0.05        # 5% aspect ratio
            )
        ...
        if len(scored_matches) > 1:
            ...
            print(f"   🥈 Runner-up: {runner_up.template_name} (score: {scored_matches[1][0]:.3f}, conf: {runner_up.confidence:.3f})")
    ...
    def _calculate_template_quality_score(self, template_name: str) -> float:
        ...
        if any(term in template_name.lower() for term in ["possession", "territory", "triangle", "arrow"]):
            return 0.6
    ...
    def calculate_overlap(self, box1: Tuple[int, int, int, int], 
    ...
    def save_template(self, name: str, template: np.ndarray, 
                     direction: Direction, triangle_type: TriangleType):
    ...
    def create_template_debug_visualization(self, frame: np.ndarray, 
    ...
    def add_custom_template_from_roi(self, hud_img: np.ndarray, roi: Tuple[int, int, int, int],
                                   name: str, direction: Direction, triangle_type: TriangleType):
        """
