...
def visualize_detected_triangles(roi_img, contours, results, title, output_filename):
    ...
    for i, (contour, result) in enumerate(zip(contours, results)):
        ...
        if result.is_valid:
            ...
            if result.direction.value == "left":
                cv2.arrowedLine(
                    vis_img, (center[0] + 15, center[1]), (center[0] - 15, center[1]), color, 2
                )
            elif result.direction.value == "right":
                cv2.arrowedLine(
                    vis_img, (center[0] - 15, center[1]), (center[0] + 15, center[1]), color, 2
                ...
                cv2.arrowedLine(
                    vis_img, (center[0], center[1] + 15), (center[0], center[1] - 15), color, 2
                )
            elif result.direction.value == "down":
                cv2.arrowedLine(
                    vis_img, (center[0], center[1] - 15), (center[0], center[1] + 15), color, 2
    ...
    if valid_count > 0:
        cv2.putText(
            vis_img,
            f"SUCCESS: {valid_count} TRIANGLES FOUND!",
            (10, 60),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.7,
            (0, 255, 0),
        ...
        cv2.putText(
            vis_img, "NO VALID TRIANGLES", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2
        )
...
def main():
    ...
    for i, contour in enumerate(territory_contours):
        ...
        if result.is_valid:
            print(
                f"      Direction: {result.direction.value.upper()}, Confidence: {result.confidence:.1%}"
            )
