...
def test_yolo_integrated_template_matching():
    ...
    for detection in triangle_detections:
        ...
        class_name = detection['class']
        conf = detection['confidence']
        print(f"  📍 {class_name}: bbox=({int(x1)}, {int(y1)}, {int(x2)}, {int(y2)}) size=({w}x{h}) conf={conf:.3f}")
    ...
    if matches:
        ...
        for i, match in enumerate(matches, 1):
            print(f"   {i}. {match.triangle_type.value} {match.direction.value} - conf: {match.confidence:.3f} scale: {match.scale_factor:.2f}x")
