...
class DownTemplateCropperGUI:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            control_frame, text="SpygateAI Down Template Creator", font=("Arial", 14, "bold")
        )
        title_label.pack(pady=(0, 10))

        subtitle_label = ttk.Label(
            control_frame,
            text="Enhanced with Optimal OCR Preprocessing",
            font=("Arial", 10),
            foreground="green",
        )
        ...
        ttk.Label(control_frame, text="📸 Screenshots:", font=("Arial", 12, "bold")).pack(
            anchor=tk.W
        )
        ...
        ttk.Label(
            goal_frame,
            text="💡 Use 3rd.png (GOAL screenshot) for all",
            font=("Arial", 9),
            foreground="orange",
        ).pack(anchor=tk.W, padx=5)

        # Preprocessing option
        ttk.Label(control_frame, text="🔧 Processing:", font=("Arial", 12, "bold")).pack(
            anchor=tk.W, pady=(20, 5)
        )

        self.preprocessing_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            control_frame, text="Apply SpygateAI OCR preprocessing", variable=self.preprocessing_var
        ).pack(anchor=tk.W, padx=10)

        ttk.Label(
            control_frame,
            text="(Scale=3.5x, CLAHE, Gamma=0.8, etc.)",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor=tk.W, padx=20)

        # Zoom controls
        ttk.Label(control_frame, text="🔍 Zoom Controls:", font=("Arial", 12, "bold")).pack(
            anchor=tk.W, pady=(20, 5)
        )
        ...
        ttk.Button(zoom_frame, text="➕ Zoom In", command=self.zoom_in, width=12).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(zoom_frame, text="➖ Zoom Out", command=self.zoom_out, width=12).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(zoom_frame, text="🔄 Reset", command=self.reset_zoom, width=8).pack(
            side=tk.LEFT, padx=2
        )
        ...
        ttk.Label(
            control_frame,
            text="💡 Mouse wheel to zoom, right-click drag to pan",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor=tk.W, padx=10)
        
        # Instructions
        ttk.Label(control_frame, text="📋 Instructions:", font=("Arial", 12, "bold")).pack(
            anchor=tk.W, pady=(20, 5)
        )
        ...
        for instruction in instructions:
            if instruction.startswith("📏") or instruction.startswith("🥅"):
                ttk.Label(
                    control_frame, text=instruction, font=("Arial", 10, "bold"), foreground="blue"
                ).pack(anchor=tk.W, padx=10)
            elif instruction == "":
                ttk.Label(control_frame, text="", font=("Arial", 2)).pack()
            else:
                ttk.Label(control_frame, text=instruction, font=("Arial", 9)).pack(
                    anchor=tk.W, padx=15
                )
        ...
        ttk.Button(button_frame, text="🔄 Reset Crop", command=self.reset_crop).pack(
            fill=tk.X, pady=2
        )
        ttk.Button(
            button_frame, text="🔍 Preview Processing", command=self.preview_processing
        ).pack(fill=tk.X, pady=2)
        ttk.Button(
            button_frame, text="🧪 Test Preprocessing", command=self.test_preprocessing
        ).pack(fill=tk.X, pady=2)
        ttk.Button(button_frame, text="📁 Export All", command=self.export_templates).pack(
            fill=tk.X, pady=2
        )
        ...
        status_label = ttk.Label(
            control_frame, textvariable=self.status_var, font=("Arial", 10), foreground="blue"
        )
        status_label.pack(pady=(20, 0))
        
        # Progress
        ttk.Label(control_frame, text="📊 Progress:", font=("Arial", 12, "bold")).pack(
            anchor=tk.W, pady=(20, 5)
        )
        ...
        try:
            ...
            if abs(actual_size[0] - expected_size[0]) < 50:  # Allow some tolerance
                pipeline_checks.append(
                    f"✅ Scaling (3.5x): OK - {actual_size[0]}x{actual_size[1]}px"
                )
            else:
                pipeline_checks.append(
                    f"❌ Scaling (3.5x): FAILED - {actual_size[0]}x{actual_size[1]}px"
                )
            ...
            if len(unique_values) <= 10 and (0 in unique_values or 255 in unique_values):
                ...
                pipeline_checks.append(
                    f"❌ Thresholding: FAILED - {len(unique_values)} unique values"
                )
            ...
            ttk.Label(
                test_window,
                text="🧪 Preprocessing Pipeline Verification",
                font=("Arial", 14, "bold"),
            ).pack(pady=10)
            ...
            for step in steps:
                ttk.Label(steps_frame, text=step, font=("Arial", 10)).pack(
            ...
            for check in pipeline_checks:
                ...
                ttk.Label(results_frame, text=check, font=("Arial", 10), foreground=color).pack(
                    anchor=tk.W, padx=10, pady=2
                )
            ...
            ttk.Label(
                test_window,
                text="Pipeline working! Templates will be optimized for SpygateAI detection.",
                font=("Arial", 10),
                foreground="green",
            ).pack(pady=10)
    ...
    def end_crop(self, event):
        ...
        # Validate crop
        if (
            abs(self.crop_end[0] - self.crop_start[0]) < 10
            or abs(self.crop_end[1] - self.crop_start[1]) < 10
        ):
    ...
    def save_template(self):
        ...
        try:
            ...
            print(
                f"Base scale: {base_scale:.3f}, Zoom: {self.zoom_factor:.3f}, Final scale: {self.scale_factor:.3f}"
            )
            ...
            if self.preprocessing_var.get():
                ...
                if processed_template is not None:
                    ...
                    processing_info = (
                        "Enhanced with SpygateAI OCR preprocessing (Scale=3.5x, CLAHE, Gamma=0.8)"
                    )
                else:
            ...
            self.status_var.set(
                f"✅ Saved {down_type} template: {template.shape[1]}x{template.shape[0]}px ({processing_info})"
            )
    ...
    def preview_processing(self):
        ...
        try:
            ...
            # Info label
            info_text = (
                "SpygateAI Optimal Pipeline: Scale=3.5x → CLAHE → Gamma=0.8 → "
                "Blur → Adaptive Threshold → Morphological Closing"
            )
            ttk.Label(preview_window, text=info_text, font=("Arial", 9), foreground="green").pack(
                pady=5
            )
