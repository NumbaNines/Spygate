...
class FPTester:
    ...
    def filter_by_size_and_position(self, detections, img_shape):
        ...
        for det in detections:
            ...
            if limits:
                if not (
                    limits.get("min_w", 0) <= width <= limits.get("max_w", w)
                    and limits.get("min_h", 0) <= height <= limits.get("max_h", h)
                ):
                    size_valid = False
...
def test_current_model():
    ...
    test_images_dir = Path(
        "hud_region_training/hud_region_training_8class/datasets_8class/val/images"
    )
    ...
    # Summary
    reduction_percent = (
        ((total_original - total_filtered) / total_original * 100) if total_original > 0 else 0
    )
