...
class ScreenCapture:
    ...
    def __init__(self, monitor=1, region=None):
        ...
        if self.method is None:
            raise RuntimeError(
                "❌ No screen capture method available! Install: pip install mss pyautogui"
            )
...
def live_detection(
    model, conf_threshold=0.15, monitor=1, region=None, save_video=None, show_preview=True
):
    ...
    try:
        while True:
            ...
            cv2.putText(
                frame, info_text, (10, overlay_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
            )
            overlay_y += 35
            ...
            for i, (class_name, count) in enumerate(detection_stats.items()):
                if count > 0:
                    ...
                    if "triangle_area" in class_name:
                        ...
                        text = f"{class_name}: {count}"
                    cv2.putText(
                        frame, text, (10, overlay_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2
                    )
                    overlay_y += 25
            ...
            if frame_count % 100 == 0:
                ...
                triangle_rate = (
                    (triangle_stats["both_detected"] / frame_count) * 100 if frame_count > 0 else 0
                )
                print(
                    f"Frame {frame_count} | FPS: {avg_fps:.1f} | Total: {total_detections} | 🔺 Both triangles: {triangle_rate:.1f}%"
                )
...
def print_triangle_statistics(triangle_stats, frame_count):
    ...
    possession_rate = (
        (triangle_stats["possession_detected"] / frame_count) * 100 if frame_count > 0 else 0
    )
    territory_rate = (
        (triangle_stats["territory_detected"] / frame_count) * 100 if frame_count > 0 else 0
    )
    both_rate = (triangle_stats["both_detected"] / frame_count) * 100 if frame_count > 0 else 0

    print(
        f"🔺 Possession Triangle: {triangle_stats['possession_detected']} frames ({possession_rate:.1f}%)"
    )
    print(
        f"🔺 Territory Triangle: {triangle_stats['territory_detected']} frames ({territory_rate:.1f}%)"
    )
...
def print_final_statistics(detection_stats, frame_count, fps_queue, triangle_stats=None):
    ...
    if triangle_stats:
        ...
        print(
            f"{status_p} Possession Triangle: {triangle_stats['possession_detected']} frames ({possession_rate:.1f}%)"
        )
        print(
            f"{status_t} Territory Triangle: {triangle_stats['territory_detected']} frames ({territory_rate:.1f}%)"
        )
        print(
            f"{status_b} Both Triangles: {triangle_stats['both_detected']} frames ({both_rate:.1f}%)"
        )
    ...
    for class_name, count in detection_stats.items():
        if count > 0:
            ...
            if "triangle_area" in class_name:
                ...
                symbol = ""
            print(
                f"{status} {symbol} {class_name:<20}: {count:>4} detections ({rate:.1f}% of frames)"
            )
        else:
            symbol = "🔺" if "triangle_area" in class_name else ""
...
def list_available_monitors():
    """List all available monitors and let user choose."""
    if not MSS_AVAILABLE:
        print("❌ MSS not available, cannot list monitors")
        return 1
    ...
    try:
        with mss.mss() as sct:
            ...
            while True:
                try:
                    choice = input(
                        f"\nSelect monitor (1-{len(monitors)-1}) or 'q' to quit: "
                    ).strip()
...
def show_monitor_preview(monitor_num):
    """Show a preview of the selected monitor for 3 seconds."""
    if not MSS_AVAILABLE:
        return

    try:
        capture = ScreenCapture(monitor=monitor_num)
        print(f"\n📺 Showing preview of Monitor {monitor_num} for 3 seconds...")
        print("Press any key to continue or wait for auto-continue...")
        ...
        while time.time() - start_time < 3:
            ...
            if frame is not None:
                ...
                if cv2.waitKey(100) & 0xFF != 255:  # Any key pressed
                    break
            else:
                break
...
def main():
    ...
    parser.add_argument(
        "--conf", "-c", type=float, default=0.15, help="Confidence threshold (default: 0.15)"
    )
    parser.add_argument(
        "--monitor",
        "-m",
        type=int,
        help="Monitor number to capture (if not specified, will show selection menu)",
    )
    parser.add_argument(
        "--region",
        "-r",
    ...
    # If just listing monitors, do that and exit
    ...
    if monitor_num is None:
        ...
        monitor_num = list_available_monitors()
        if monitor_num is None:
            return

        # Show preview of selected monitor
        try:
            preview_choice = (
                input("\nWould you like to see a preview of the selected monitor? (y/n): ")
                .strip()
                .lower()
            )
            if preview_choice in ["y", "yes"]:
                show_monitor_preview(monitor_num)
        except (EOFError, KeyboardInterrupt):
            # Skip preview if input is piped or interrupted
            print("Skipping preview...")
            pass

    print(f"\n🎯 Starting detection on Monitor {monitor_num}...")
