...
from .serializers import ReleaseSerializer, DownloadSerializer
from apps.analysis.models import GameAnalysis, Formation, Play, Situation
from apps.analysis.serializers import GameAnalysisSerializer, FormationSerializer, PlaySerializer, SituationSerializer
...
class ReleaseViewSet(viewsets.ModelViewSet):
    ...
    def download(self, request, pk=None):
        ...
        download = Download.objects.create(
            user=request.user,
            release=release,
            file_size=release.installer.size
        )
...
class PlayViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def by_formation(self, request):
        ...
        if not formation_id:
            return Response(
                {"error": "formation_id parameter is required"},
...
class SituationViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def by_down(self, request):
        ...
        if not down:
            return Response(
                {"error": "down parameter is required"},
                status=status.HTTP_400_BAD_REQUEST
            )
