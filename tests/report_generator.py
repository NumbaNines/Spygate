...
class TestReportGenerator:
    ...
    def generate_tracking_report(
        ...
        report = {
            ...
            "baseline_comparison": (
                self._compare_with_baseline(tracking_metrics, performance_metrics, baseline_metrics)
                if baseline_metrics
                else None
            ),
            "overall_scores": self._calculate_overall_scores(tracking_metrics, performance_metrics),
        }
    ...
    def generate_benchmark_report(
        ...
        report = {
            ...
            "baseline_comparison": (
                self._compare_benchmarks(benchmark_results, baseline_results)
                if baseline_results
                else None
            ),
    ...
    def _generate_metadata(
        self, custom_metadata: Optional[dict[str, str]] = None
    ) -> dict[str, str]:
                ...
                comparison[f"{category}_comparison"] = {
                    metric: {
                        ...
                        "percent_change": (
                            ...
                            / baseline_metrics[category].get(metric, 1.0)
                            * 100
                        ),
                    }
    ...
    def _calculate_overall_scores(
        ...
        scores["overall_score"] = np.mean([scores["tracking_score"], scores["performance_score"]])
                ...
                comparison[category] = {
                    metric: {
                        ...
                        "percent_change": (
                            ...
                            / baseline_results[category].get(metric, 1.0)
                            * 100
                        ),
                    }
    ...
    def _generate_visualizations(self, report: dict) -> None:
        ...
        for category, metrics in report["tracking_metrics"].items():
            if "accuracy" in category.lower():
                self.visualizer.plot_tracking_accuracy(metrics, title=f"{category} Metrics")
            elif "position" in category.lower():
                self.visualizer.plot_position_errors(metrics, title=f"{category} Metrics")
            elif "velocity" in category.lower():
                self.visualizer.plot_velocity_errors(metrics, title=f"{category} Metrics")
            elif "occlusion" in category.lower():
                self.visualizer.plot_occlusion_metrics(metrics, title=f"{category} Metrics")
            elif "formation" in category.lower():
                self.visualizer.plot_formation_metrics(metrics, title=f"{category} Metrics")
        ...
        # Plot summary
        self.visualizer.plot_summary_report(report, title="Overall Performance Summary")
    ...
    def _generate_benchmark_visualizations(self, report: dict) -> None:
        ...
        self.visualizer.plot_performance_metrics(
            report["performance_analysis"], title="Performance Analysis"
        )

    def save_report(self, report: dict, filename: str = "test_report.json") -> None:
        """Save report to file.
    ...
    def save_benchmark_report(self, report: dict, filename: str = "benchmark_report.json") -> None:
