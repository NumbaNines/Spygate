...
def test_complete_integration():
    ...
    for i, frame_data in enumerate(test_frames):
        ...
        result = integrated_system.process_frame(
            frame_data["frame"], frame_data["processed_image"], frame_data["raw_game_state"]
        )
    ...
    for i, result in enumerate(clips_created):
        ...
        confidence = result.confidence_breakdown
        print(
            f"   Clip {i+1}: {clip.play_down} & {clip.play_distance} "
    ...
    for i, result in enumerate(clips_created):
        ...
        if (
            preserved_state.get("down") == clip.play_down
            and preserved_state.get("distance") == clip.play_distance
        ):
