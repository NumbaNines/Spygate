...
import pytest
from PyQt6.QtCore import QEvent, QMimeData, QPoint, QPointF, Qt, QThread, QUrl
...
def test_drag_enter_with_mixed_files(video_import_widget, qtbot):
    ...
    mime_data = QMimeData()
    mime_data.setUrls([QUrl.fromLocalFile("test.mp4"), QUrl.fromLocalFile("test.txt")])
...
def test_worker_with_multiple_files(video_import_worker, qtbot, tmp_path):
    ...
    worker = VideoImportWorker(
        [video_import_worker.file_paths[0], str(invalid_file)]  # Valid file  # Invalid file
    )
