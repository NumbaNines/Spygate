...
def main():
    ...
    try:
        with open("template_matching_results.json", "r") as f:
            results = json.load(f)
        ...
        if template_details:
            ...
            if possession_matches:
                ...
                for match in possession_matches[:3]:  # Show top 3
                    print(f"      - {match['direction']} arrow: {match['confidence']:.3f} ({match['template']})")
            ...
            if territory_matches:
                ...
                for match in territory_matches[:3]:  # Show top 3
                    print(f"      - {match['direction']} triangle: {match['confidence']:.3f} ({match['template']})")
