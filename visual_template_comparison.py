    ...
    for test_case in test_cases:
        ...
        for template_name, template_data in detector.templates.items():
            ...
            if (
                ...
                and template_img.shape[1] <= preprocessed.shape[1]
            ):

                result = cv2.matchTemplate(preprocessed, template_img, cv2.TM_CCOEFF_NORMED)
        ...
        for i, match in enumerate(best_matches[:3]):
            ...
            print(
                f"      {i+1}. {status} {match['template']}: {match['confidence']:.3f} (down {match['down']})"
            )

        # Test actual detector
        result = detector.detect_down_in_yolo_region(
            frame, bbox, "goal" in test_case["desc"].lower()
        )
