...
class GameDetector:
        ...
        try:
            ...
            if stable_result.confidence >= self._confidence_threshold:
                ...
                self._detection_stats["avg_confidence"] = (
                    self._detection_stats["avg_confidence"]
                    * (self._detection_stats["total_detections"] - 1)
                    + stable_result.confidence
                ) / self._detection_stats["total_detections"]
    ...
    def _template_match_confidence(self, frame: np.ndarray, version: GameVersion) -> float:
        ...
        try:
            ...
            if y2 <= frame.shape[0] and x2 <= frame.shape[1] and y1 >= 0 and x1 >= 0:
    ...
    def _combine_detection_results(self, results: list[DetectionResult]) -> DetectionResult:
        ...
        method_weights = {"template_matching": 0.4, "hud_analysis": 0.4, "ml_classification": 0.6}
        ...
        return DetectionResult(
            ...
            metadata={
                "individual_results": [
                    {"method": r.detection_method, "confidence": r.confidence} for r in results
                ],
    ...
    def _calculate_game_confidence(self, image: Image.Image, version: GameVersion) -> float:
        ...
        try:
            ...
            score_region = (
                frame_array[50:100, 50:300]
                if frame_array.shape[0] > 100 and frame_array.shape[1] > 300
                else frame_array
            )
            ...
            if score_region.size > 0:
                gray_score = (
                    cv2.cvtColor(score_region, cv2.COLOR_RGB2GRAY)
                    if len(score_region.shape) == 3
                    else score_region
                )
    ...
    def convert_to_target_game(
        self, universal_data: UniversalGameData, target_game: GameVersion
    ) -> dict:
        """
        ...
        if universal_data.formation_family:
            ...
            if formation:
                ...
                conversion_notes.append(
                    f"Formation {universal_data.formation_family} not available in {target_game.value}"
                )
        ...
        if universal_data.strategic_concept:
            ...
            if strategy:
                ...
                conversion_notes.append(
                    f"Strategy {universal_data.strategic_concept} not available in {target_game.value}"
                )
    ...
    def _map_universal_to_formation(
        self, universal_formation: str, target_game: GameVersion
    ) -> Optional[str]:
    ...
    def _map_universal_to_strategy(
        self, universal_strategy: str, target_game: GameVersion
    ) -> Optional[str]:
    ...
    def _map_universal_to_situation(
        self, universal_situation: dict, target_game: GameVersion
    ) -> dict:
    ...
    def get_performance_stats(self) -> dict:
        ...
        cache_hit_rate = (
            ...
            if self._detection_stats["total_detections"] > 0
            else 0.0
        )
        ...
        return {
            ...
            "template_detection_rate": (
                self._detection_stats["template_detections"]
                / self._detection_stats["total_detections"]
                if self._detection_stats["total_detections"] > 0
                else 0.0
            ),
    ...
    def get_cross_game_compatibility(
        self, source_game: GameVersion, target_game: GameVersion
    ) -> dict:
        """
        ...
        return {
            ...
            "cross_game_mapping_supported": (
                "cross_game_mapping" in source_profile.supported_features
                and "cross_game_mapping" in target_profile.supported_features
            ),
        }
