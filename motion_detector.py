...
class MotionEventLogger:
    ...
    def log_event(self, timestamp, roi, magnitude, snapshot_path=None):
        ...
        if self.db_path:
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
            c.execute('''INSERT INTO motion_events (timestamp, roi, magnitude, snapshot_path) VALUES (?, ?, ?, ?)''',
...
class MotionDetector:
    ...
    def draw_motion_boxes(self, frame: np.ndarray, contours: list, color: tuple = (0, 255, 0), thickness: int = 2) -> np.ndarray:
    ...
    def draw_motion_contours(self, frame: np.ndarray, contours: list, color: tuple = (0, 0, 255), thickness: int = 2) -> np.ndarray:
        """Draw contours of detected motion areas on the frame."""
        output = frame.copy()
        cv2.drawContours(output, contours, -1, color, thickness)
        return output

    def draw_motion_trails(self, frame: np.ndarray, trails: list, color: tuple = (255, 0, 0), thickness: int = 2) -> np.ndarray:
    ...
    def generate_motion_heatmap(self, frame_shape: tuple, motion_points: list, alpha: float = 0.5) -> np.ndarray:
        ...
        heatmap_color = cv2.addWeighted(heatmap_color, alpha, np.zeros_like(heatmap_color), 1 - alpha, 0)
