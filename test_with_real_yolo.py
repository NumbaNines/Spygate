...
def test_with_real_yolo():
    ...
    detector = DownTemplateDetector(
        templates_dir=Path("down_templates_real"), debug_output_dir=Path("debug_real_yolo_test")
    )
    ...
    for i, test_case in enumerate(test_cases):
        ...
        try:
            ...
            print(
                f"   ✓ YOLO found down_distance_area: conf={yolo_confidence:.3f}, bbox={yolo_bbox}"
            )
    ...
    print(
        f"YOLO Detection Rate: {yolo_found_count}/{total_tests} ({yolo_found_count/total_tests*100:.1f}%)"
    )
    ...
    for result in results:
        ...
        print(
            f"  {status} {yolo_status} {result['test_case']}: {result['detected']} (conf: {conf_str}, template: {template_str})"
        )
