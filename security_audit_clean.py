...
class SecurityAuditor:
                ...
                try:
                    ...
                    for table_name in db_info["tables"][:10]:  # Limit for performance
                        try:
                            ...
                            if sensitive_cols:
                                db_info["issues"].append(f"Sensitive columns in {table_name}: {sensitive_cols}")
    ...
    def _create_audit_summary(self, audit_results: Dict) -> Dict:
        ...
        db_issues = sum(len(db.get("issues", [])) for db in 
                       audit_results.get("database_security", {}).get("databases_found", []))
        
        net_issues = sum(len(config.get("issues", [])) for config in 
                        audit_results.get("network_security", {}).get("config_files", []))
        
        summary["total_issues"] = dep_issues + file_issues + db_issues + net_issues
        summary["files_scanned"] = audit_results.get("file_permissions", {}).get("total_files_checked", 0)
