...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                           ...
                           QSpinBox, QGroupBox, QDialog, QVBoxLayout)
from PyQt6.QtCore import Qt, QTimer, QDateTime, QRect
from PyQt6.QtGui import QImage, QPixmap, QColor, QScreen
import os
import json
import mss
import csv
...
class SpygateDetector(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.viz_checkboxes = {}
        for option in ['Show Boxes', 'Show Labels', 'Show Confidence', 'Highlight No-Huddle', 'Track Motion']:
            cb = QCheckBox(option)
            cb.setChecked(option.lower().replace('-', '_').replace(' ', '_') in self.visualization_options)
    ...
    def toggle_recording(self):
        if not self.recording:
            ...
            self.csv_file = open(self.output_dir / 'data' / f'detections_{timestamp}.csv', 'w', newline='')
            self.csv_writer = csv.writer(self.csv_file)
            self.csv_writer.writerow(['timestamp', 'class', 'confidence', 'x', 'y', 'width', 'height'])
        ...
        try:
            ...
            if self.frame_counter > self.skip_frames:
                ...
                results = self.model.predict(
                    source=frame,
                    conf=float(self.conf_combo.currentText()),
                    verbose=False
                )
            ...
            if self.last_detection_results is not None:
                ...
                if self.visualization_options['track_motion']:
                    ...
                    if hasattr(self, 'prev_frame'):
                        ...
                        motion_mask = cv2.threshold(cv2.cvtColor(frame_diff, cv2.COLOR_BGR2GRAY), 30, 255, cv2.THRESH_BINARY)[1]
                        annotated_frame = cv2.addWeighted(annotated_frame, 1, cv2.cvtColor(motion_mask, cv2.COLOR_GRAY2BGR), 0.3, 0)
