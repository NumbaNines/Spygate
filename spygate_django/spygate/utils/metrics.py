    ...
    def get_gauge_stats(
        ...
        with self._lock:
            ...
            if window:
                ...
                values = [v for v, t in zip(values, timestamps) if t >= cutoff]
            ...
            for gauge_file in gauge_dir.glob("*.json"):
                try:
                    with open(gauge_file) as f:
                        data = json.load(f)
                        name = data["name"]
                        self._gauges[name] = deque(data["values"], maxlen=self.max_history)
                        self._timestamps[f"gauge_{name}"] = deque(
                            data["timestamps"], maxlen=self.max_history
                        )
                except Exception as e:
            ...
            for event_file in event_dir.glob("*.json"):
                try:
                    with open(event_file) as f:
                        data = json.load(f)
                        name = data["name"]
                        self._events[name] = deque(data["events"], maxlen=self.max_history)
                except Exception as e:
            ...
            for hist_file in hist_dir.glob("*.json"):
                try:
                    with open(hist_file) as f:
                        data = json.load(f)
                        name = data["name"]
                        self._histograms[name] = deque(data["values"], maxlen=self.max_history)
