...
def visualize_rejected_contours(roi_img, contours, results, title, output_filename):
    ...
    cv2.putText(vis_img, "REJECTED CONTOURS:", (text_x, y_offset), 
    ...
    for i, (contour, result) in enumerate(zip(contours, results)):
        ...
        # Contour info
        cv2.putText(vis_img, f"Contour {i+1}:", (text_x, y_offset), 
    ...
    # Add color legend
    y_offset += 20
    cv2.putText(vis_img, "COLOR LEGEND:", (text_x, y_offset), 
               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    y_offset += 20
