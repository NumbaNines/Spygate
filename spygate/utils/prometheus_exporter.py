...
class PrometheusExporter:
    ...
    def __init__(
        ...
        # Create metrics
        self.fps = Gauge(f"{prefix}fps", "Current frames per second", registry=self.registry)

        self.memory_usage = Gauge(
            f"{prefix}memory_usage_mb", "Current memory usage in MB", registry=self.registry
        )

        self.gpu_memory = Gauge(
            f"{prefix}gpu_memory_mb", "Current GPU memory usage in MB", registry=self.registry
        )

        self.quality_level = Gauge(
            f"{prefix}quality_level", "Current quality level (0.0-1.0)", registry=self.registry
        )
