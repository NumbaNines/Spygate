...
from tests.utils import Frame, BoundingBox, Position, Color
        ...
        for seq_idx in range(num_sequences):
            ...
            for frame_idx in range(frames_per_sequence):
                ...
                cv2.rectangle(
                    frame,
                    (x, y),
                    (x + object_size[0], y + object_size[1]),
                    (255, 255, 255),
                    -1
                )
        ...
        for seq_idx in range(num_sequences):
            ...
            for frame_idx in range(frames_per_sequence):
                ...
                if frame_idx < collision_frame:
                    ...
                    progress = (frame_idx - collision_frame) / (frames_per_sequence - collision_frame)
                ...
                # Draw objects
                cv2.rectangle(frame, (x1_curr, y1_curr),
                            (x1_curr + object_size[0], y1_curr + object_size[1]),
                            (255, 0, 0), -1)
                cv2.rectangle(frame, (x2_curr, y2_curr),
                            (x2_curr + object_size[0], y2_curr + object_size[1]),
                            (0, 255, 0), -1)
                
                # Save frame
                frame_path = self.output_dir / f'occlusion_seq_{seq_idx:03d}_frame_{frame_idx:03d}.png'
                cv2.imwrite(str(frame_path), frame)
            ...
            dataset['ground_truth'].append({
                'object1': ground_truth_obj1,
                'object2': ground_truth_obj2
            })
    ...
    def generate_formation_dataset(
        ...
        for seq_idx in range(num_sequences):
            ...
            for frame_idx in range(frames_per_sequence):
                ...
                # Save frame
                frame_path = self.output_dir / f'formation_seq_{seq_idx:03d}_frame_{frame_idx:03d}.png'
                cv2.imwrite(str(frame_path), frame)
