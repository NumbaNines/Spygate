...
def analyze_false_positive():
    ...
    for i, contour in enumerate(contours):
        ...
        if area > 50:  # Only analyze significant contours
            ...
            print(
                f"    Aspect: {aspect:.3f} (territory needs {detector.TERRITORY_MIN_ASPECT}-{detector.TERRITORY_MAX_ASPECT})"
            )
            ...
            if area_valid and aspect_valid and points_valid:
                ...
                if result.is_valid:
                    ...
                    for point in contour:
                        ...
                        dist = np.abs(
                            np.cross(
                                top_bottom_line[1] - top_bottom_line[0], point - top_bottom_line[0]
                            )
                    ...
                    cv2.circle(
                        contour_vis, tuple(base_point), 3, (255, 255, 0), -1
                    )  # Cyan for base
                    cv2.circle(
                        contour_vis, result.center, 3, (255, 0, 255), -1
                    )  # Magenta for center
