#!/usr/bin/env python3
"""
CLAHE Optimizer for PaddleOCR (GPU Accelerated)
...
class CLAHEOptimizerGPU:
    def __init__(self):
        """Initialize the CLAHE optimizer with GPU acceleration"""
        self.samples_dir = Path("preprocessing_test_samples")
        self.results_dir = Path("clahe_optimization_results_gpu")
    ...
    def run_optimization(self):
        """Run the complete CLAHE optimization with GPU acceleration"""
        print("🔍 CLAHE OPTIMIZATION FOR PADDLEOCR (GPU ACCELERATED)")
        print("=" * 60)
        ...
        for clip_limit in self.clip_limits:
            for grid_size in self.grid_sizes:
                ...
                for i, image_path in enumerate(sorted(sample_images), 1):
                    ...
                    eta = (
                        (elapsed / current_test) * (total_tests - current_test)
                        if current_test > 0
                        else 0
                    )
                    ...
                    if result:
                        ...
                        print(
                            ...
                            f"Total: {result['processing_time']:.2f}s "
                            f"(CLAHE: {result['clahe_time']:.3f}s, OCR: {result['ocr_time']:.2f}s)"
                        )
                    else:
                        print(f"    ❌ Failed")
                ...
                if combination_results:
                    ...
                    avg_total_time = np.mean([r["processing_time"] for r in combination_results])
                    avg_clahe_time = np.mean([r["clahe_time"] for r in combination_results])
                    avg_ocr_time = np.mean([r["ocr_time"] for r in combination_results])

                    print(f"  📊 Combination Summary (completed in {combination_time:.1f}s):")
                    print(f"      Average Confidence: {avg_conf:.3f}")
                    print(f"      Average Text Count: {avg_texts:.1f}")
                    print(f"      Average Total Time: {avg_total_time:.2f}s")
                    print(f"      Average CLAHE Time: {avg_clahe_time:.3f}s")
                    print(f"      Average OCR Time: {avg_ocr_time:.2f}s")
                print()

        total_elapsed = time.time() - self.start_time
        print(f"✅ Completed {len(self.results)} successful tests in {total_elapsed:.1f}s")
        print(f"⚡ Average speed: {total_elapsed/len(self.results):.2f}s per test")
    ...
    def save_results(self):
        ...
        results_file = self.results_dir / f"clahe_optimization_gpu_results_{timestamp}.json"
        ...
        performance_summary = {
            ...
            "avg_time_per_test": total_time / len(self.results) if self.results else 0,
            "gpu_enabled": True,
        }

        output_data = {"performance_summary": performance_summary, "results": self.results}
    ...
    def analyze_results(self):
        ...
        print("\n📊 CLAHE OPTIMIZATION ANALYSIS (GPU ACCELERATED)")
        print("=" * 60)

        # Performance statistics
        total_time = time.time() - self.start_time if self.start_time else 0
        avg_time_per_test = total_time / len(self.results)

        print(f"⚡ PERFORMANCE STATISTICS:")
        print(f"   Total processing time: {total_time:.1f}s")
        print(f"   Average time per test: {avg_time_per_test:.2f}s")
        print(f"   Tests per minute: {60/avg_time_per_test:.1f}")
        print()
        ...
        total_times = [r["processing_time"] for r in self.results]
        clahe_times = [r["clahe_time"] for r in self.results]
        ocr_times = [r["ocr_time"] for r in self.results]

        print(f"   Average confidence: {np.mean(confidences):.3f}")
        print(f"   Average text count: {np.mean(text_counts):.1f}")
        print(f"   Average total time: {np.mean(total_times):.3f}s")
        print(f"   Average CLAHE time: {np.mean(clahe_times):.3f}s")
        print(f"   Average OCR time: {np.mean(ocr_times):.3f}s")
        print()
        ...
        # Save summary
        summary_file = self.results_dir / "clahe_optimization_gpu_summary.txt"
        with open(summary_file, "w") as f:
            f.write("CLAHE Optimization Summary (GPU Accelerated)\n")
            f.write("=" * 45 + "\n\n")
            f.write(f"Total processing time: {total_time:.1f}s\n")
            f.write(f"Average time per test: {avg_time_per_test:.2f}s\n")
...
def main():
    ...
    print("🚀 Starting CLAHE Optimization for PaddleOCR (GPU Accelerated)")
    ...
    optimizer = CLAHEOptimizerGPU()
    optimizer.run_optimization()

    print("\n✅ CLAHE OPTIMIZATION COMPLETE!")
    print("📁 Check the 'clahe_optimization_results_gpu' folder for:")
    print("   - Detailed JSON results with performance metrics")
