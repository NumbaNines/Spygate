...
class TriangleDetectionTester:
    ...
    def analyze_image(self, image_path: Path) -> dict[str, Any]:
        ...
        try:
            ...
            detection_details = self._process_detections(
                detections[0] if detections else None, image
            )
            ...
            error_result = {"image_name": image_path.name, "error": str(e), "status": "error"}
            print(f"   ❌ Error: {str(e)}")
            return error_result
    ...
    def _process_detections(self, detection_result, image: np.ndarray) -> dict[str, Any]:
        ...
        details = {
            ...
            "bounding_boxes": [],
            "confidences": [],
            "triangle_analysis": {"possession_triangles": [], "territory_triangles": []},
        }
        ...
        for i in range(len(boxes)):
            ...
            # Get class name
            class_name = (
                self.analyzer.ui_classes[class_id]
                if class_id < len(self.analyzer.ui_classes)
    ...
    def _create_visualization(
        self, image: np.ndarray, detection_details: dict[str, Any]
    ) -> np.ndarray:
        ...
        # Draw bounding boxes
        for i, (class_name, bbox, conf) in enumerate(
            zip(
            ...
            cv2.rectangle(
                vis_image, (x1, y1 - label_size[1] - 10), (x1 + label_size[0], y1), color, -1
            )
            cv2.putText(
                vis_image, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
            )
        ...
        if successful:
            ...
            for result in successful:
                possession_count += len(
                    result["detections"]["triangle_analysis"]["possession_triangles"]
                )
                territory_count += len(
                    result["detections"]["triangle_analysis"]["territory_triangles"]
                )
