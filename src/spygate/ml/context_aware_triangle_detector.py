...
from .triangle_orientation_detector import Direction, TriangleOrientationDetector, TriangleType
...
class TriangleCandidate:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and abs(self.area - other.area) < 1.0
            and self.triangle_type == other.triangle_type
        )
...
class ContextAwareTriangleDetector:
    ...
    def detect_triangles_with_context(
        self, frame: np.ndarray, hud_region: tuple[int, int, int, int]
        ...
        # Debug visualization
        if self.debug_output_dir:
            self.create_context_debug_visualization(hud_img, all_candidates, validated_candidates)

        return validated_candidates

    def super_loose_detection(
        ...
        for contour in unique_contours:
            ...
            if not (
                self.SUPER_LOOSE_THRESHOLDS["MIN_AREA"]
                <= area
                <= self.SUPER_LOOSE_THRESHOLDS["MAX_AREA"]
            ):
                continue
            ...
            if not (
                self.SUPER_LOOSE_THRESHOLDS["MIN_ASPECT"]
                <= aspect_ratio
                <= self.SUPER_LOOSE_THRESHOLDS["MAX_ASPECT"]
            ):
                continue
    ...
    def validate_with_context(self, candidate: TriangleCandidate, hud_img: np.ndarray) -> float:
        ...
        total_score = sum(scores[key] * self.CONTEXT_WEIGHTS[key] for key in scores)
    ...
    def check_ocr_consistency(self, candidate: TriangleCandidate, hud_img: np.ndarray) -> float:
        ...
        if candidate.triangle_type == TriangleType.POSSESSION:
            ...
            if (
                self.current_game_state.away_score is not None
                and self.current_game_state.home_score is not None
            ):
                score += 0.2
    ...
    def check_game_logic(self, candidate: TriangleCandidate) -> float:
        ...
        if self.previous_game_state:
            ...
            if candidate.triangle_type == TriangleType.POSSESSION and (
                self.current_game_state.away_score != self.previous_game_state.away_score
                or self.current_game_state.home_score != self.previous_game_state.home_score
            ):
                return 0.9

            # Yard line change suggests territory triangle update
            if (
                candidate.triangle_type == TriangleType.TERRITORY
                and self.current_game_state.yard_line != self.previous_game_state.yard_line
            ):
                return 0.9
    ...
    def create_context_debug_visualization(
        self,
        hud_img: np.ndarray,
        ...
        for info in state_info:
            cv2.putText(
                vis_img, info, (text_x, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1
            )
            y_offset += 20
