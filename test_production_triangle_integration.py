...
def test_production_integration():
    ...
    try:
        ...
        analyzer = EnhancedGameAnalyzer(
            hardware=hardware, debug_output_dir=Path("debug_production_integration")
        )
        ...
        for i, img_path in enumerate(test_images, 1):
            ...
            if hasattr(analyzer, "clip_queue") and analyzer.clip_queue:
                ...
                for clip in analyzer.clip_queue[-2:]:  # Show last 2
                    print(
                        f"   - {clip['description']} ({clip['pre_buffer_seconds']}s pre, {clip['post_buffer_seconds']}s post)"
                    )
        ...
        print("-" * 30)
        print(
            f"Total key moments: {len(analyzer.key_moments) if hasattr(analyzer, 'key_moments') else 0}"
        )
        print(
            f"Total clips queued: {len(analyzer.clip_queue) if hasattr(analyzer, 'clip_queue') else 0}"
        )
