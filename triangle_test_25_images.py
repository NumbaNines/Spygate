...
def main():
    ...
    try:
        ...
        template_detector = YOLOIntegratedTriangleDetector(
            game_analyzer=analyzer, debug_output_dir=Path("debug_template_matching")
        )
            ...
            try:
                ...
                if template_matches:
                    for match in template_matches:
                        print(
                            f"      - {match.triangle_type.value} {match.direction.value}: {match.confidence:.3f} (scale: {match.scale_factor:.1f}x)"
                        )
        ...
        possession_matches = sum(
            1 for r in results for t in r["template_details"] if t["type"] == "possession"
        )
        territory_matches = sum(
            1 for r in results for t in r["template_details"] if t["type"] == "territory"
        )
