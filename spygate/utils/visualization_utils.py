    ...
    if use_gradient:
        ...
        if gradient_stops is None:
            gradient_stops = [
                ...
                (0.5, (min(color[0] * 1.5, 255), min(color[1] * 1.5, 255), min(color[2] * 1.5, 255))),
                (1.0, (255, 255, 255))
            ]
...
def draw_motion_vectors(
    ...
    for i, (start, end, magnitude) in enumerate(zip(starts, ends, magnitudes)):
        ...
        if smooth_arrows:
            ...
            cv2.arrowedLine(frame, tuple(map(int, start)), tuple(map(int, end)), 
                          tuple(map(int, col)), thickness, tipLength=0.2)
    ...
    def visualize_ball_prediction(
        ...
        for i, ((x, y), conf) in enumerate(zip(predictions, confidences)):
            ...
            if uncertainty_radius:
                uncertainty = int(radius * (1 + (1 - conf) * 2))  # Larger radius for lower confidence
                overlay = result.copy()
            ...
            if i == len(predictions) - 1:
                ...
                # Draw text
                cv2.putText(result, conf_text, (text_x, text_y),
