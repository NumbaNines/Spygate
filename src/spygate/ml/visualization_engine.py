...
class DetectionVisualizer:
    ...
    def draw_detection(
        self, image: np.ndarray, bbox: list[int], class_name: str, confidence: float
    ) -> np.ndarray:
    ...
    def draw_ocr_result(
        self, image: np.ndarray, text: str, bbox: list[int], confidence: float
    ) -> np.ndarray:
        ...
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(image, label, (x, y - 4), font, self.config.font_scale * 0.8, color, 1)

        return image

    def draw_triangle_detection(
        self, image: np.ndarray, bbox: list[int], direction: str, confidence: float
    ) -> np.ndarray:
        ...
        if direction == "right":
            cv2.arrowedLine(image, (x + w // 4, y + h // 2), (x + 3 * w // 4, y + h // 2), color, 2)
        elif direction == "left":
            cv2.arrowedLine(image, (x + 3 * w // 4, y + h // 2), (x + w // 4, y + h // 2), color, 2)
        elif direction == "up":
            cv2.arrowedLine(image, (x + w // 2, y + 3 * h // 4), (x + w // 2, y + h // 4), color, 2)
        elif direction == "down":
            cv2.arrowedLine(image, (x + w // 2, y + h // 4), (x + w // 2, y + 3 * h // 4), color, 2)
    ...
    def create_visualization(
        ...
        triangle_detections: Optional[list[dict]] = None,
    ) -> np.ndarray:
        ...
        for det in detections:
            vis_image = self.draw_detection(
                vis_image, det["bbox"], det["class_name"], det["confidence"]
            )
        ...
        if ocr_results and self.config.show_ocr:
            for ocr in ocr_results:
                vis_image = self.draw_ocr_result(
                    vis_image, ocr["text"], ocr["bbox"], ocr["confidence"]
                )
        ...
        if triangle_detections and self.config.show_triangles:
            for tri in triangle_detections:
                vis_image = self.draw_triangle_detection(
                    vis_image, tri["bbox"], tri["direction"], tri["confidence"]
                )
    ...
    def visualize_triangle_geometry(
        self,
        frame: np.ndarray,
        triangle_points: list[np.ndarray],
        validation_results: list[tuple[bool, float]],
    ) -> np.ndarray:
    ...
    def visualize_ocr_regions(self, frame: np.ndarray, ocr_results: dict[str, dict]) -> np.ndarray:
    ...
    def visualize_hud_analysis(self, frame: np.ndarray, game_state: "GameState") -> np.ndarray:
    ...
    def _draw_validation_metrics(
        self, overlay: np.ndarray, points: np.ndarray, confidence: float
    ) -> None:
    ...
    def save_visualizations(self, visualizations: list[np.ndarray], base_path: Path) -> None:
    ...
    def display_visualizations(self, visualizations: list[np.ndarray]) -> None:
        ...
        titles = ["Raw Frame", "Detections", "Triangle Geometry", "OCR Regions", "HUD Analysis"]
