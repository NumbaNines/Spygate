...
class TestGPUMemoryManagement:
    ...
    def test_memory_cleanup_and_fragmentation(self):
        ...
        if TORCH_AVAILABLE and torch.cuda.is_available():
            ...
            logger.info(
                f"Initial GPU memory usage: {initial_stats.get('usage_percentage', 0):.2f}%"
            )
            ...
            logger.info(
                f"After allocation GPU memory usage: {after_alloc_stats.get('usage_percentage', 0):.2f}%"
            )
            ...
            logger.info(
                f"After cleanup GPU memory usage: {after_cleanup_stats.get('usage_percentage', 0):.2f}%"
            )
