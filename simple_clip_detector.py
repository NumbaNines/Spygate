...
class SimpleClipDetector:
    ...
    def _validate_game_state(self, frame_number: int, game_state: dict[str, Any], timestamp: float) -> PlayState:
        ...
        if confidence < self.min_confidence_threshold:
            validation_issues.append(f"Confidence {confidence} below threshold {self.min_confidence_threshold}")
    ...
    def _detect_new_play_with_validation(self, validated_state: PlayState) -> Optional[ClipInfo]:
        ...
        if self.last_validated_down != current_down:
            print(f"🏈 VALIDATED PLAY CHANGE: {self.last_validated_down} → {current_down} at frame {validated_state.frame}")
            self.last_validated_down = current_down
            return self._create_clip_info_validated(validated_state)
    ...
    def _create_clip_info_validated(self, play_state: PlayState) -> ClipInfo:
        ...
        print(f"🎬 VALIDATED CLIP CREATED: {clip_info.play_down} & {clip_info.play_distance} (confidence: {clip_info.validation_confidence:.2f})")
        return clip_info
    ...
    def _finalize_previous_clips_smart(self, new_play_frame: int):
        ...
        for clip in self.active_clips:
            if clip.status == "pending":
                ...
                print(f"   - Duration: {duration:.1f}s (start: {clip.start_frame}, end: {clip.end_frame})")
        ...
        for clip in self.active_clips:
            if clip.status == "pending":
                ...
                if clip.end_frame is None or provisional_end > clip.end_frame:
                    ...
                    print(f"🔄 Extended clip due to OCR failure: {clip.play_down} & {clip.play_distance}")
    ...
    def finalize_remaining_clips(self, final_frame: int):
        ...
        for clip in self.active_clips:
            if clip.status == "pending":
                ...
                duration = (clip.end_frame - clip.start_frame) / self.fps
                print(f"🏁 FINAL CLIP FINALIZED: {clip.play_down} & {clip.play_distance} (duration: {duration:.1f}s)")
