...
class ClipThumbnailWidget(QWidget):
    ...
    def approve_clip(self):
        ...
        """)
        print(f"✅ Approved clip: {self.clip.situation} ({self.clip.start_time:.1f}s-{self.clip.end_time:.1f}s)")
    ...
    def reject_clip(self):
        ...
        """)
        print(f"❌ Rejected clip: {self.clip.situation} ({self.clip.start_time:.1f}s-{self.clip.end_time:.1f}s)")
...
class ClipViewerWidget(QWidget):
    ...
    def export_clips_to_directory(self, clips: List[DetectedClip], export_dir: str):
        ...
        for i, clip in enumerate(clips):
            output_path = os.path.join(export_dir, f"clip_{i+1}_{clip.situation.replace(' ', '_')}.mp4")
        ...
        try:
            ...
            for i in range(0, 100, 2):
                ...
                if i > 20 and i % 15 == 0:  # Detect clips every 15%
                    ...
                    situations = ["3rd & Long", "Red Zone", "Turnover", "Touchdown", "Big Play", "Goal Line Stand"]
...
class SpygateDesktopAppWithViewer(QMainWindow):
    ...
    def on_clips_detected(self, video_path: str, clips_data: List[Dict]):
        ...
        self.status_bar.showMessage(f"✅ Detected {len(clips_data)} clips from {os.path.basename(video_path)}")
