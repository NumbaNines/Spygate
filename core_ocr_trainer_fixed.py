#!/usr/bin/env python3
"""
Fixed OCR trainer for core dataset.
Corrected model dimensions and architecture.
"""
...
class CoreOCRModel(nn.Module):
    def __init__(self, vocab_size, hidden_size=256, num_layers=2):
        ...
        # CNN feature extractor (fixed for 128x32 images)
        ...
        # Calculate correct input size for RNN
        # After CNN: [B, 512, 2, 8] -> reshape to [B, 8, 512*2]
        rnn_input_size = 512 * 2  # 1024
    ...
    def forward(self, x):
        # CNN features
        features = self.cnn(x)  # [B, 512, 2, 8]

        # Reshape for RNN
        B, C, H, W = features.shape
        features = features.permute(0, 3, 1, 2)  # [B, W, C, H]
        features = features.reshape(B, W, C * H)  # [B, 8, 512*2]

        # RNN
        rnn_out, _ = self.rnn(features)  # [B, 8, hidden_size*2]
        rnn_out = self.dropout(rnn_out)

        # Classification
        output = self.classifier(rnn_out)  # [B, 8, vocab_size]

        return output


def train_core_ocr():
    print("🚀 Training Core OCR Model (Fixed)")
    print("=" * 50)
    ...
    # Create datasets
    train_dataset = CoreOCRDataset(
        train_data, char_to_idx, max_length=8
    )  # Reduced for 8 time steps
    val_dataset = CoreOCRDataset(val_data, char_to_idx, max_length=8)
