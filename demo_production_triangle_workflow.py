...
def demo_production_workflow():
    ...
    for state in game_states:
        print(f"\n📸 Frame {state['frame']}:")
        print(f"   🏈 Possession: {state['possession']} ({'Away' if state['possession'] == 'left' else 'Home'} team)")
        print(f"   🗺️ Territory: {state['territory']} ({'Opponent' if state['territory'] == 'up' else 'Own'} territory)")
