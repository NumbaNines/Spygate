...
class TutorialManager:
    ...
    def __init__(self):
        ...
        self.tutorial_path = os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'tutorials')
    ...
    def get_next_tutorial(self) -> Optional[TutorialInfo]:
        ...
        for tutorial in self.tutorials.values():
            if not self.is_tutorial_completed(tutorial.id):
                ...
                if all(self.check_feature_available(feature) for feature in tutorial.required_features):
                    return tutorial
        return None
