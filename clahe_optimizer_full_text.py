...
def test_clahe_parameters():
    """Test different CLAHE parameters with COMPLETE OCR text output"""
    ...
    print(f"📁 Found {len(image_files)} sample images (LANCZOS4 scaled)")
        ...
        for clip_limit in clip_limits:
            for grid_size in grid_sizes:
                ...
                try:
                    ...
                    # Calculate metrics and extract ALL text
                ...
                # Progress update with ALL detected text
                ...
                # Display ALL detected text (no truncation)
                if (
                    detected_texts
                    and detected_texts[0] != "No text detected"
                    and detected_texts[0] != "OCR Error"
                ):
                    print(f"   📝 ALL detected text:")
                    for i, text in enumerate(detected_texts, 1):
                        print(f"      {i:2d}. {text}")
                else:
                    print(f"   📝 {detected_texts[0] if detected_texts else 'No text detected'}")

    # Analyze results
    print(f"\n{'='*80}")
    print("🎯 CLAHE OPTIMIZATION RESULTS (LANCZOS4 SCALED - COMPLETE TEXT)")
    print(f"{'='*80}")
    ...
    print(f"   📝 Complete text list:")
    for i, text in enumerate(best_by_detections["detected_texts"], 1):
        print(f"      {i:2d}. {text}")
    ...
    print(f"   📝 Complete text list:")
    for i, text in enumerate(best_by_confidence["detected_texts"], 1):
        print(f"      {i:2d}. {text}")
    ...
    print(f"   📝 Complete text list:")
    for i, text in enumerate(best_by_text_length["detected_texts"], 1):
        print(f"      {i:2d}. {text}")
    ...
    with open("clahe_optimization_results_complete_text.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n💾 Detailed results saved to 'clahe_optimization_results_complete_text.json'")
