...
class TrackingMetrics:
    ...
    def calculate_position_error(
        ...
        errors = [
            np.linalg.norm(np.array(pred) - np.array(gt))
    ...
    def calculate_velocity_error(
        ...
        metrics = {
            ...
            "mean_direction_error_deg": (
                np.degrees(np.mean(direction_errors)) if direction_errors else 0.0
            ),
        }
    ...
    def calculate_occlusion_metrics(
        ...
        metrics = {
            "occlusion_accuracy": (
                occlusion_success / len(occlusion_frames) if occlusion_frames else 1.0
            ),
            "non_occlusion_accuracy": (
                non_occlusion_success / len(non_occlusion_ious) if non_occlusion_ious else 1.0
            ),
    ...
    def calculate_formation_metrics(
        ...
        for pred_frame, gt_frame in zip(predicted_positions, ground_truth_positions):
            ...
            structure_error = np.mean(
                [np.linalg.norm(pred - gt) for pred, gt in zip(pred_relative, gt_relative)]
            )
    ...
    def generate_summary_report(
        ...
        weights = {"tracking": 0.4, "position": 0.3, "occlusion": 0.2, "performance": 0.1}
