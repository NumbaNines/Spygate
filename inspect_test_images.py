...
def show_image_comparison():
    ...
    test_cases = [
        ...
        (
            ...
            "down_templates_real/4TH_GOAL.png",
            "4TH_GOAL vs 4th_goal",
        ),
    ]
    ...
    for i, (test_path, template_path, title) in enumerate(test_cases):
        ...
        axes[i, 0].set_title(
            f"Test: {Path(test_path).name} ({test_img_rgb.shape[1]}x{test_img_rgb.shape[0]})"
        )
        axes[i, 0].axis("off")
        ...
        axes[i, 1].set_title(
            f"Template: {Path(template_path).name} ({template_img_rgb.shape[1]}x{template_img_rgb.shape[0]})"
        )
        axes[i, 1].axis("off")
