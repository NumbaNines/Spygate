...
def test_detection_accuracy(detector):
    ...
    for test_case in test_cases:
        ...
        if result:
            ...
            print(f"   {'✅' if correct else '❌'} {test_case['text']}: "
...
def test_context_awareness(detector):
    ...
    normal_correct = (normal_result and 
                     normal_result.situation_type.value == "normal")
    ...
    goal_correct = (goal_result and 
                   goal_result.situation_type.value == "goal")
...
def generate_recommendation(accuracy, avg_confidence, avg_time, context_score, edge_score):
    ...
    overall_score = (
        accuracy * 0.30 +           # 30% weight on accuracy
        (avg_confidence) * 0.20 +   # 20% weight on confidence
        (1.0 if avg_time < 50 else 0.5) * 0.20 +  # 20% weight on performance
        context_score * 0.15 +      # 15% weight on context awareness
        edge_score * 0.15           # 15% weight on edge case handling
    )
