...
def create_motion_sequence(
    ...
    for i in range(num_frames):
        ...
        cv2.rectangle(
            frame,
            (x_pos, y_pos),
            (x_pos + object_size[0], y_pos + object_size[1]),
            color,
            -1
        )
        frames.append(frame)
...
def create_occlusion_sequence(
    ...
    for i in range(num_frames):
        ...
        # Draw objects
        cv2.rectangle(frame, (x1_pos, y1_pos),
                     (x1_pos + object_size[0], y1_pos + object_size[1]),
                     (255, 0, 0), -1)
        cv2.rectangle(frame, (x2_pos, y2_pos),
...
def mock_hardware_environment(
    has_gpu: bool = True,
    available_memory: float = 8192.0,
    gpu_memory: float = 4096.0
) -> Dict:
