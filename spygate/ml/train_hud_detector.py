...
class CustomLoss(nn.Module):
    ...
    def __init__(self):
        ...
        self.cls_loss = nn.BCEWithLogitsLoss(reduction='mean', pos_weight=torch.tensor([1.5]))  # Weight positive samples
        self.text_reg_loss = nn.BCEWithLogitsLoss(reduction='mean', pos_weight=torch.tensor([2.0]))  # Higher weight for text
    ...
    def forward(
        ...
        total_loss = (
            7.5 * box_loss +  # Critical for accurate detection
            0.5 * cls_loss +  # Less weight as classes are simpler
            1.5 * text_loss +  # Important for text detection
            1.0 * angle_loss +  # Moderate weight for angle accuracy
            0.5 * angle_conf_loss +  # Lower weight for confidence
            1.0 * conf_loss  # Balanced overall confidence
        )
...
def train_model(
    ...
    model = CustomYOLO11(
        cfg="yolov11.yaml" if not model_path else model_path,
        nc=10  # Number of HUD element classes
    )
