...
class DatasetOrganizer:
    ...
    def find_annotated_pairs(self) -> List[Tuple[str, str]]:
        ...
        png_files_source = {f for f in os.listdir(self.source_dir) 
                           if f.lower().endswith('.png')}
        png_files_unannotated = set()
        
        if os.path.exists(unannotated_dir):
            png_files_unannotated = {f for f in os.listdir(unannotated_dir) 
                                   if f.lower().endswith('.png')}
        ...
        if os.path.exists(unannotated_dir):
            for png_file in png_files_unannotated:
                ...
                if txt_file not in txt_files and os.path.exists(os.path.join(unannotated_dir, png_file)):
    ...
    def split_dataset(self, annotated_pairs: List[Tuple[str, str]]) -> None:
        ...
        for split_name, pairs in splits.items():
            for img_path, txt_path in pairs:
                try:
                    # Move image
                    dst_img = os.path.join(self.base_path, "test_dataset", "images", 
                                         split_name, os.path.basename(img_path))
                    shutil.copy2(img_path, dst_img)
                    
                    # Move annotation
                    dst_txt = os.path.join(self.base_path, "test_dataset", "labels", 
                                         split_name, os.path.basename(txt_path))
