...
def create_simple_visualization(image, game_state):
    ...
    if hasattr(game_state, 'down') and game_state.down:
        cv2.putText(vis_image, f"Down: {game_state.down}", (10, y_offset), font, font_scale, color, thickness)
        y_offset += 30
    
    if hasattr(game_state, 'distance') and game_state.distance:
        cv2.putText(vis_image, f"Distance: {game_state.distance}", (10, y_offset), font, font_scale, color, thickness)
        y_offset += 30
        
    if hasattr(game_state, 'possession_team') and game_state.possession_team:
        cv2.putText(vis_image, f"Possession: {game_state.possession_team}", (10, y_offset), font, font_scale, color, thickness)
        y_offset += 30
        
    if hasattr(game_state, 'confidence') and game_state.confidence:
        cv2.putText(vis_image, f"Confidence: {game_state.confidence:.2f}", (10, y_offset), font, font_scale, color, thickness)
        y_offset += 30
    
    return vis_image

def main():
    parser = argparse.ArgumentParser(description="Unified Madden visualization tool (advanced pipeline)")
    parser.add_argument("input_dir", help="Directory containing images to process")
    parser.add_argument("--output-dir", default="visualization_results", help="Directory to save results")
