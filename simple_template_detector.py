...
class DownTemplateEngine:
    ...
    def match_down_templates(self, region: np.ndarray, content_type: ContentType) -> List[DownTemplateMatch]:
        ...
        for template_name, template_data in self.templates.items():
            ...
            if confidence > 0.3:  # Lower threshold for real templates
                matches.append(DownTemplateMatch(
    ...
    def _match_single_down_template(self, region: np.ndarray, template: np.ndarray) -> Tuple[float, Tuple[int, int, int, int]]:
...
class DistanceOCREngine:
    ...
    def extract_distance_from_region(self, region: np.ndarray, down_location: Tuple[int, int, int, int]) -> Tuple[Optional[int], str, float]:
        """
    ...
    def detect_down_distance(self, hud_region: np.ndarray) -> DetectionResult:
        ...
        if not down_matches:
            return DetectionResult(
                ...
                method_used="no_down_template_match"
            )
        ...
        if distance is not None:
            raw_text = f"{best_down_match.down} & {distance}" if distance > 0 else f"{best_down_match.down} & GOAL"
...
def create_down_templates():
    ...
    for down_str in down_strings:
        ...
        cv2.putText(raw_template, down_str, (2, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        ...
        cv2.putText(streamer_template, down_str, (1, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
...
def test_template_detector():
    ...
    cv2.putText(streamer_test, "3RD & 7", (1, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
