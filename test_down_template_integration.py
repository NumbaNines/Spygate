...
class DownTemplateIntegrationTest:
    ...
    def _test_template_loading(self) -> dict:
        ...
        logger.info(
            f"📊 Loaded {results['templates_loaded']} templates "
    ...
    def _test_frame_detection(self) -> dict:
        ...
        frame_files = list(self.test_frames_dir.glob("*.jpg")) + list(
            self.test_frames_dir.glob("*.png")
        )
                ...
                results["detection_details"].append(
                    {
                        ...
                        "confidence": (
                            frame_result.template_result.confidence
                            if frame_result.template_result
                            else 0.0
                        ),
        ...
        logger.info(
            f"🎯 Detection results: {results['successful_detections']}/{results['frames_tested']} successful"
        )
    ...
    def _mock_ocr_detection(
        ...
        return {"text": "1ST & 10", "confidence": 0.75, "method": "mock_ocr"}
    ...
    def _benchmark_performance(self) -> dict:
        ...
        logger.info(
            f"🏃‍♂️ Template: {results['template_avg_ms']:.2f}ms, "
    ...
    def _test_edge_cases(self) -> dict:
        ...
        logger.info(
            f"🛡️ Edge cases: {results['edge_cases_passed']}/{results['total_edge_cases']} passed"
        )
    ...
    def _generate_summary(self) -> dict:
        ...
        avg_confidence = np.mean(
            [r.template_result.confidence for r in self.test_results if r.template_result]
        )
    ...
    def _generate_report(self, results: dict) -> None:
        ...
        with open(report_file, "w") as f:
            ...
            f.write(
                f"- **Validation Status:** {'✅ PASSED' if tv.get('validation_passed') else '❌ FAILED'}\n\n"
            )
            ...
            f.write(
                f"- **Template Matching:** {pb.get('template_avg_ms', 0):.2f}ms ± {pb.get('template_std_ms', 0):.2f}ms\n"
            )
            f.write(
                f"- **OCR Processing:** {pb.get('ocr_avg_ms', 0):.2f}ms ± {pb.get('ocr_std_ms', 0):.2f}ms\n"
            )
            f.write(f"- **Speedup Factor:** {pb.get('speedup_factor', 0):.2f}x\n")
            f.write(
                f"- **Template Faster:** {'✅ YES' if pb.get('template_faster') else '❌ NO'}\n\n"
            )
            ...
            f.write(
                f"- **Average Processing Time:** {summary.get('average_processing_time_ms', 0):.2f}ms\n"
            )
