...
class GammaAdaptiveThresholdOptimizer:
    ...
    def __init__(self):
        ...
        # Initialize PaddleOCR (CPU mode for stability)
    ...
    def apply_gamma_correction(self, image, gamma):
        ...
        if gamma == 0.0:
            # Special case: gamma=0 means no correction (return original)
            return image
    ...
    def process_single_combination(self, image_path, gamma, block_size, c_value):
        """Process image with specific gamma and adaptive threshold parameters"""
        try:
            # Load image
            ...
            if gamma == 0.0:
                gamma_image = clahe_image  # No gamma correction
            else:
    ...
    def test_all_combinations(self, input_dir, output_dir, max_images=5):
        """Test all gamma and adaptive threshold combinations"""
        input_path = Path(input_dir)
        if not input_path.exists():
            print(f"Input directory does not exist: {input_dir}")
            return []

        # Get image files (limit to max_images for testing)
...
def main():
    ...
    results = optimizer.test_all_combinations(
        "preprocessing_test_samples",
        "gamma_adaptive_optimization_results",
        max_images=5,  # Test on 5 images for speed
    )
