...
def create_arrow_contour(direction: str) -> np.ndarray:
    ...
    if direction == "right":
        points = np.array(
            [[[0, 10]], [[30, 10]], [[20, 0]], [[30, 10]], [[20, 20]], [[0, 10]]], dtype=np.int32
        )
    else:  # left
        points = np.array(
            [[[30, 10]], [[0, 10]], [[10, 0]], [[0, 10]], [[10, 20]], [[30, 10]]], dtype=np.int32
        )
    return points
...
def create_triangle_contour(direction: str) -> np.ndarray:
    ...
    if direction == "up":
        points = np.array([[[15, 0]], [[30, 30]], [[0, 30]]], dtype=np.int32)
    else:  # down
        points = np.array([[[15, 30]], [[30, 0]], [[0, 0]]], dtype=np.int32)
    return points
