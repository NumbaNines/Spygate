...
class OCRClipDetectorIntegration:
    ...
    def process_frame(
        self, frame_number: int, processed_image, raw_game_state: dict[str, Any]
    ) -> Optional[IntegratedClipResult]:
        """
    ...
    def _create_clean_game_state(
        self, ocr_result: Optional[dict], raw_game_state: dict
    ) -> dict[str, Any]:
        """
    ...
    def _create_integrated_result(
        self, clip_info: ClipInfo, ocr_result: Optional[dict], clean_game_state: dict
    ) -> IntegratedClipResult:
        ...
        enhancement_details = {
            ...
            "clip_detection": {
                ...
                "boundaries": {"start": clip_info.start_frame, "end": clip_info.end_frame},
    ...
    def _calculate_confidence_breakdown(
        self, ocr_result: Optional[dict], clip_info: ClipInfo
    ) -> dict[str, float]:
    ...
    def get_integration_stats(self) -> dict[str, Any]:
        ...
        if stats["ocr_extractions"] > 0:
            stats["ensemble_correction_rate"] = (
                stats["ensemble_corrections"] / stats["ocr_extractions"]
            )
            stats["temporal_correction_rate"] = (
                stats["temporal_corrections"] / stats["ocr_extractions"]
            )
    ...
    def print_integration_summary(self):
        ...
        print(
            f"🔍 OCR Extractions: {stats['ocr_extractions']:,} ({stats.get('ocr_extraction_rate', 0):.1%})"
        )
        print(
            f"🎬 Clips Created: {stats['clips_created']:,} ({stats.get('clip_creation_rate', 0):.1%})"
        )
        print(
            f"⭐ High Confidence: {stats['high_confidence_clips']:,} ({stats.get('high_confidence_rate', 0):.1%})"
        )

        print(f"\n🚀 OCR ENHANCEMENTS:")
        print(
            f"   Ensemble Corrections: {stats['ensemble_corrections']:,} ({stats.get('ensemble_correction_rate', 0):.1%})"
        )
        print(
            f"   Temporal Corrections: {stats['temporal_corrections']:,} ({stats.get('temporal_correction_rate', 0):.1%})"
        )
        print(
            f"   Validation Boosts: {stats['validation_boosts']:,} ({stats.get('validation_boost_rate', 0):.1%})"
        )
