...
class MultiMethodDownDetector:
    ...
    def detect_down_professional(self, frame: np.ndarray, hud_bbox: Tuple, 
                               possession_triangle_bbox: Optional[Tuple] = None,
                               territory_triangle_bbox: Optional[Tuple] = None) -> DetectionResult:
        """
        ...
        print(f"   Method 1 (OCR): {ocr_result.down} & {ocr_result.distance} (conf: {ocr_result.confidence:.3f})")
        
        # Method 5: Game State Context Analysis
        context_result = self._method_5_game_state_context(ocr_result)
        print(f"   Method 5 (Context): {context_result.down} & {context_result.distance} (conf: {context_result.confidence:.3f})")
        
        # Method 6: Possession/Territory State Validation
        possession_result = self._method_6_possession_territory_validation(ocr_result, possession_triangle_bbox, territory_triangle_bbox)
        print(f"   Method 6 (Possession): {possession_result.down} & {possession_result.distance} (conf: {possession_result.confidence:.3f})")
        
        # Combine all methods
        final_result = self._combine_all_methods(ocr_result, context_result, possession_result)
        print(f"   🏆 Final Result: {final_result.down} & {final_result.distance} (conf: {final_result.confidence:.3f})")
    ...
    def _method_1_enhanced_ocr(self, frame: np.ndarray, hud_bbox: Tuple, 
                             possession_triangle_bbox: Optional[Tuple] = None) -> DetectionResult:
        """
    ...
    def _method_6_possession_territory_validation(self, ocr_result: DetectionResult,
                                                possession_bbox: Optional[Tuple],
                                                territory_bbox: Optional[Tuple]) -> DetectionResult:
        """
        Method 6: Possession/Territory State Validation
    ...
    def _calculate_adaptive_region(self, frame: np.ndarray, hud_bbox: Tuple, 
                                 possession_triangle_bbox: Optional[Tuple] = None) -> Optional[np.ndarray]:
        """
    ...
    def _preprocess_for_ocr(self, region: np.ndarray) -> np.ndarray:
        ...
        scale_factor = 5
        scaled = cv2.resize(region, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)
            ...
            if match:
                try:
                    ...
                    if 1 <= down_num <= 4:
                        ...
                        if "2" in ocr_text and ("nd" in ocr_text.lower() or "rd" in ocr_text.lower()):
                        ...
                        elif "3" in ocr_text and ("nd" in ocr_text.lower() or "rd" in ocr_text.lower()):
    ...
    def _combine_all_methods(self, ocr_result: DetectionResult, 
                           context_result: DetectionResult,
                           possession_result: DetectionResult) -> DetectionResult:
        """
        ...
        if ocr_result.down == context_result.down == possession_result.down:
            agreements = 3
        elif (ocr_result.down == context_result.down or 
              ocr_result.down == possession_result.down or
              context_result.down == possession_result.down):
            agreements = 2
