...
def capture_and_visualize():
    ...
    for text in overlay_text:
        cv2.putText(
            vis_image, text, (10, overlay_y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2
        )
...
def create_detailed_hud_breakdown(hud_regions, output_path="hud_breakdown.jpg"):
    ...
    for i, hud_region in enumerate(hud_regions):
        ...
        cv2.putText(
            grid_image,
            label,
            (x_offset, y_offset + max_height + 20),
        ...
        cv2.putText(
            grid_image,
            element_text,
            (x_offset, y_offset + max_height + 40),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.5,
            (200, 200, 200),
            1,
    ...
    for triangle in results["triangles"]:
        if triangle["type"] == "field_position_context":
            ...
            for line in text_lines:
                cv2.putText(
                    viz_image, line, (10, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2
                )
                y_pos += 25
        else:
            ...
            cv2.putText(viz_image, label, (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
