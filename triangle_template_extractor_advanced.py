...
class AdvancedTriangleExtractor:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if self.is_valid_image_coord(img_x, img_y):
                ...
                if clicked_point_index >= 0:
                    ...
                    print(f"🎯 Selected point {clicked_point_index + 1}: ({self.polygon_points[clicked_point_index][0]}, {self.polygon_points[clicked_point_index][1]})")
                else:
                    # Add new point
                    self.polygon_points.append((img_x, img_y))
                    self.selected_point_idx = len(self.polygon_points) - 1
                    print(f"📍 Added point {len(self.polygon_points)}: ({img_x}, {img_y}) - Total points: {len(self.polygon_points)}")
            ...
            if self.dragging:
                self.dragging = False
                self.drag_start = None
                print(f"✅ Moved point {self.selected_point_idx + 1} to: ({self.polygon_points[self.selected_point_idx][0]}, {self.polygon_points[self.selected_point_idx][1]})")
        
        elif event == cv2.EVENT_MOUSEMOVE:
            # Drag selected point
            if self.dragging and self.selected_point_idx >= 0 and self.is_valid_image_coord(img_x, img_y):
    ...
    def update_display(self):
        ...
        if len(self.polygon_points) > 0:
            ...
            for pt in self.polygon_points:
                ...
                if (0 <= disp_pt[0] < display_copy.shape[1] and 
                    0 <= disp_pt[1] < display_copy.shape[0]):
                    ...
                    # Draw point number
                    cv2.putText(display_copy, str(original_index + 1), 
        ...
        if self.current_mask is not None:
            ...
            if mask_region.size > 0:
                mask_resized = cv2.resize(mask_region, (display_copy.shape[1], display_copy.shape[0]), 
        ...
        # Add status info
        cv2.putText(display_copy, f"Zoom: {self.zoom_factor:.2f}x", 
                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(display_copy, f"Points: {len(self.polygon_points)}", 
                   (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        if self.selected_point_idx >= 0:
            cv2.putText(display_copy, f"Selected: Point {self.selected_point_idx + 1}", 
                       (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        
        if self.dragging:
            cv2.putText(display_copy, "DRAGGING - Move mouse to reposition", 
    ...
    def extract_and_save_template(self):
        ...
        template_display = cv2.resize(template, (template.shape[1]*4, template.shape[0]*4), 
                                    interpolation=cv2.INTER_NEAREST)
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                if self.selected_point_idx >= 0 and len(self.polygon_points) > 0:
                    ...
                    print(f"🗑️ Removed point {self.selected_point_idx + 1}: ({removed_point[0]}, {removed_point[1]})")
