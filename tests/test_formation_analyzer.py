...
from spygate.video.formation_analyzer import (
    FormationAnalyzer,
    FormationConfig,
    FormationType,
)
...
class TestFormationAnalyzer(unittest.TestCase):
    ...
    def test_analyze_formation_not_enough_players(self, mock_detect):
        ...
        mock_detect.return_value = [
            {"bbox": np.array([100, 100, 150, 150]), "confidence": 0.9}
            for _ in range(5)
        ]
        
        result = self.analyzer.analyze_formation(
            self.frame, self.game_version, is_offense=True
        )
    ...
    def test_analyze_formation_spread(self, mock_detect):
        ...
        for x, y in spread_positions:
            ...
            mock_detections.append({
                "bbox": np.array([px-25, py-25, px+25, py+25]),
                "confidence": 0.9
            })
    ...
    def test_analyze_formation_four_three(self, mock_detect):
        ...
        result = self.analyzer.analyze_formation(
            self.frame, self.game_version, is_offense=False
        )
    ...
    def test_cluster_positions(self):
        ...
        positions = [
            (0.2, 0.2), (0.21, 0.19),  # Cluster 1
            (0.5, 0.5), (0.51, 0.49),  # Cluster 2
            (0.8, 0.8), (0.79, 0.81),  # Cluster 3
        ]
