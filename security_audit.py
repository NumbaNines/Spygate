...
class SecurityAuditor:
    ...
    def _scan_dependencies(self) -> Dict:
        ...
        try:
            ...
            if req_file.exists():
                ...
                results["total_packages"] = len([l for l in lines if l.strip() and not l.startswith('#')])
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        print(f"   ✅ Safety scan completed: {len(safety_data)} vulnerabilities found")
                    else:
        ...
        try:
            ...
            python_files = [f for f in python_files if not any(
                exclude in str(f) for exclude in [
                    "venv", "env", "__pycache__", ".git", "build", "dist", 
    ...
    def _audit_file_permissions(self) -> Dict:
        ...
        try:
            sensitive_extensions = ['.py', '.sh', '.bat', '.ps1', '.conf', '.cfg', '.ini', '.json', '.yaml', '.yml']
                ...
                try:
                    ...
                    for table_name in db_info["tables"]:
                        try:
                            ...
                            sensitive_cols = [col[1] for col in columns if any(
                                keyword in col[1].lower() for keyword in 
                                ['password', 'pass', 'pwd', 'secret', 'token', 'key', 'hash']
                            )]
                            
                            if sensitive_cols:
                                db_info["issues"].append(f"Table '{table_name}' contains sensitive columns: {sensitive_cols}")
    ...
    def _create_audit_summary(self, audit_results: Dict) -> Dict:
        ...
        summary["files_scanned"] = (
            code_security.get("total_files_scanned", 0) +
            file_perms.get("total_files_checked", 0)
        )
    ...
    def save_audit_report(self, audit_results: Dict):
        ...
        try:
            ...
            print(f"   📝 Recommendations: {audit_results['audit_summary']['recommendations_count']}")
