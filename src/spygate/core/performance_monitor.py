...
class PerformanceMonitor:
    ...
    def __init__(
        ...
        logger.info(f"Initialized PerformanceMonitor with {self.hardware.tier.name} tier")
    ...
    def _check_thresholds(self):
        ...
        with self._lock:
            ...
            if (
                ...
                > self.thresholds.max_memory_mb * self.thresholds.memory_warning_threshold
            ):
                logger.warning(f"High memory usage: {self.stats['memory_usage_mb']:.1f}MB")
                self._optimize_memory()

            # Check GPU memory
            if (
                self.hardware.has_cuda
                and self.stats["gpu_memory_mb"]
                > self.thresholds.max_gpu_memory_mb * self.thresholds.gpu_warning_threshold
            ):
                logger.warning(f"High GPU memory usage: {self.stats['gpu_memory_mb']:.1f}MB")
    ...
    def _optimize_memory(self):
        ...
        if self.current_quality > self.thresholds.min_quality:
            self.current_quality = max(
                self.thresholds.min_quality, self.current_quality - self.thresholds.quality_step
            )
    ...
    def _cleanup(self):
        ...
        if (
            self.stats["fps"] > self.thresholds.target_fps * 1.1
            and self.current_quality < self.thresholds.max_quality
            and self.stats["memory_usage_mb"] < self.thresholds.max_memory_mb * 0.8
            and (
                not self.hardware.has_cuda
                or self.stats["gpu_memory_mb"] < self.thresholds.max_gpu_memory_mb * 0.8
            )
        ):
            self.current_quality = min(
                self.thresholds.max_quality, self.current_quality + self.thresholds.quality_step
            )
