...
class EnhancedDownDetector:
    ...
    def detect_down_adaptive(self, frame: np.ndarray, hud_bbox: Tuple, 
        ...
        context_result = self._validate_with_game_context(ocr_result, possession_triangle_bbox, territory_triangle_bbox)
        ...
        try:
            ...
            for down_num, template in self.number_templates.items():
                ...
                for scale in [0.8, 1.0, 1.2, 1.5]:
                    ...
                    if scaled_template.shape[0] <= gray_region.shape[0] and scaled_template.shape[1] <= gray_region.shape[1]:
    ...
    def _validate_with_game_context(self, ocr_result: DetectionResult, 
    ...
    def _combine_detection_results(self, ocr_result: DetectionResult, 
                                 template_result: DetectionResult,
                                 context_result: DetectionResult) -> DetectionResult:
        """
        ...
        if (ocr_result.down == template_result.down == context_result.down and 
            ocr_result.down is not None):
