...
class TestVisualizer:
    ...
    def plot_velocity_errors(
        ...
        magnitude_metrics = {k: v for k, v in velocity_metrics.items() if "magnitude" in k}
        direction_metrics = {k: v for k, v in velocity_metrics.items() if "direction" in k}
    ...
    def plot_formation_metrics(
        ...
        error_metrics = {k: v for k, v in formation_metrics.items() if "error" in k}
        stability_metrics = {k: v for k, v in formation_metrics.items() if "stability" in k}
    ...
    def plot_performance_metrics(
        ...
        timing_metrics = {
            k: v for k, v in performance_metrics.items() if any(x in k for x in ["time", "fps"])
        }
        memory_metrics = {k: v for k, v in performance_metrics.items() if "memory" in k}
        gpu_metrics = {k: v for k, v in performance_metrics.items() if "gpu" in k}

        num_plots = 2 + (len(gpu_metrics) > 0)
        fig, axes = plt.subplots(1, num_plots, figsize=(15, 6))

        # Plot timing metrics
        self._plot_metric_group(axes[0], timing_metrics, "Timing Metrics", baseline_metrics)

        # Plot memory metrics
        self._plot_metric_group(axes[1], memory_metrics, "Memory Metrics", baseline_metrics)

        # Plot GPU metrics if available
        if gpu_metrics:
            self._plot_metric_group(axes[2], gpu_metrics, "GPU Metrics", baseline_metrics)
    ...
    def _plot_metric_group(
        ...
        bars1 = ax.bar(
            x - width / 2 if baseline_metrics else x, list(metrics.values()), width, label="Current"
        )
    ...
    def plot_summary_report(
        ...
        for angle, value, category in zip(angles[:-1], values[:-1], categories):
            ...
            ax.text(angle, value, f"{value:.2f}", ha=ha, va="center")
    ...
    def save_metrics_report(
        self, report: dict[str, dict[str, float]], filename: str = "metrics_report.json"
    ) -> None:
