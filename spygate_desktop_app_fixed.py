...
class FixedAutoClipDetector(QObject):
        ...
        try:
            ...
            while cap.isOpened() and self.is_analyzing:
                ...
                if progress != last_progress:
                    ...
                    QTimer.singleShot(
                        0, lambda p=progress, m=message: self.analysis_progress.emit(p, m)
                    )
            ...
            # Use QTimer.singleShot for thread-safe signal emission
            QTimer.singleShot(
                0, lambda clips=self.detected_clips: self.analysis_complete.emit(clips)
            )
    ...
    def _simulate_situation_detection(
...
class FixedAutoDetectWidget(QWidget):
    ...
    def add_clip_card(self, clip_data: ClipData):
        ...
        card = QLabel(
            f"🎬 {clip_data.situation} ({clip_data.timestamp}) - {clip_data.confidence:.1%}"
        )
        card.setStyleSheet(
            """
            QLabel {
