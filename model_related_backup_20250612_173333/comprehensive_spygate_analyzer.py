...
class ComprehensiveSpygateAnalyzer:
    ...
    def __init__(self, output_dir: str = "comprehensive_analysis_results"):
        ...
        self.game_analyzer = EnhancedGameAnalyzer(hardware=self.hardware_detector)
    ...
    def detect_triangles(
        self, image: np.ndarray, yolo_detections: list[dict]
    ) -> list[dict[str, Any]]:
        """Specialized triangle detection within triangle areas."""
        triangle_detections = []

        # Find triangle areas from YOLO
        triangle_areas = [d for d in yolo_detections if "triangle" in d["class"]]
    ...
    def create_comprehensive_visualization(
        ...
        for detection in results["detections"]["yolo"]["detections"]:
            ...
            cv2.rectangle(
                vis_image, (x1, y1 - label_size[1] - 5), (x1 + label_size[0], y1), color, -1
            )
            cv2.putText(
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="comprehensive_analysis_results",
