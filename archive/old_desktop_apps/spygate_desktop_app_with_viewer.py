    ...
    def play_clip_externally(self, clip: DetectedClip):
        ...
        try:
            ...
            temp_clip_path = os.path.join(
                tempfile.gettempdir(), f"temp_clip_{clip.start_frame}.mp4"
            )
            ...
            for frame_num in range(
                clip.start_frame, min(clip.end_frame, clip.start_frame + int(fps * 10))
            ):  # Max 10 sec clips
                ret, frame = cap.read()
                if not ret:
                    break
...
class AnalysisWorker(QThread):
    ...
    def update_game_flow(self, detected_down, frame_number, ocr_text):
        ...
        print(
            f"🏈 GAME FLOW: Current={detected_down}, Expected={self.expected_next_down}, Last={self.last_confirmed_down}"
        )
    ...
    def check_for_missed_third_down(self, current_frame):
        ...
        if recent_third_downs:
            ...
            print(
                f"    ✅ Found recent 3rd down at frame {most_recent_third['frame']} ({frames_since_third} frames ago)"
            )
            return
        ...
        if recent_second_downs:
            ...
            if frames_since_second < 1800:  # Within 30 seconds (1800 frames at 60fps)
                print(
                    f"    🔍 POTENTIAL MISCLASSIFICATION: Found 2nd down at frame {most_recent_second['frame']}"
                )
                ...
                if any(x in second_text for x in ["3ND", "3D", "RD", "3R", "SD", "3RO"]):
                    ...
                    print(
                        f"       Updating frame {most_recent_second['frame']} from '2nd Down' → '3rd Down'"
                    )
        ...
        for frame_number in range(start_frame, current_frame, sample_interval):
            ...
            try:
                ...
                if len(hud_results[0].boxes) > 0:
                    ...
                    is_down, down_type = self.detect_down_and_distance_focused_third(
                        frame, hud_box, frame_number
                    )
    ...
    def detect_down_and_distance_focused_third(self, frame, hud_box, frame_number=0):
        ...
        try:
            ...
            scaled_region = cv2.resize(
                down_distance_region, (scaled_width, scaled_height), interpolation=cv2.INTER_CUBIC
            )
    ...
    def validate_down_with_game_logic(self, ocr_down, ocr_text, frame_number):
        ...
        if (
            current_yardage == 10
            and ocr_down != "1st Down"
            and self.last_yardage
            and self.last_yardage < 10
        ):
        ...
        if ocr_down == self.expected_next_down:
            print(
                f"✅ GAME LOGIC CONFIRMS: OCR '{ocr_down}' matches expected '{self.expected_next_down}'"
            )
            return ocr_down
        ...
        if self.expected_next_down == "3rd Down" and ocr_down == "2nd Down":
            ...
            if any(x in ocr_text for x in ["3ND", "3nd", "3D", "3d"]):
                ...
                print(
                    f"    OCR text '{ocr_text}' contains 3rd down indicators, correcting to 3rd Down"
                )
                return "3rd Down"
            ...
            if (
                self.last_yardage
                and current_yardage
                and current_yardage <= self.last_yardage
                and current_yardage < 10
            ):
                print(
                    f"🔧 YARDAGE LOGIC CORRECTION: Expected 3rd down, yardage progression supports this"
                )
                print(
                    f"    Yardage: {self.last_yardage} → {current_yardage}, correcting to 3rd Down"
                )
                return "3rd Down"
        ...
        print(
            f"🤷 GAME LOGIC MISMATCH: Expected '{self.expected_next_down}' but OCR says '{ocr_down}'"
        )
    ...
    def detect_down_and_distance(self, frame, hud_box, frame_number=0):
        ...
        try:
            ...
            self.recent_downs = [
                d for d in self.recent_downs if current_time - d["frame"] <= self.context_window
            ]
            self.recent_flags = [
                f for f in self.recent_flags if current_time - f["frame"] <= self.context_window
            ]
                ...
                if ocr_text:
                    ...
                    for pattern in fourth_down_patterns:
                        try:
                            if re.search(pattern, clean_text):
                                print(
                                    f"✅ 4th DOWN DETECTED! Pattern: {pattern}, Text: '{clean_text}'"
                                )
                                ...
                                validated_down = self.validate_down_with_game_logic(
                                    "4th Down", clean_text, frame_number
                                )
                    ...
                    for pattern in first_down_patterns:
                        try:
                            if re.search(pattern, clean_text):
                                print(
                                    f"✅ 1st DOWN DETECTED! Pattern: {pattern}, Text: '{clean_text}'"
                                )

                                # Validate with game logic
                                validated_down = self.validate_down_with_game_logic(
                                    "1st Down", clean_text, frame_number
                                )

                                # FORWARD DETECTION STRATEGY: Check for missed 3rd down conversions
                                self.check_for_missed_third_down_forward(
                                    frame_number, validated_down
                                )
                    ...
                    for pattern in second_down_patterns:
                        try:
                            if re.search(pattern, clean_text):
                                print(
                                    f"✅ 2nd DOWN DETECTED! Pattern: {pattern}, Text: '{clean_text}'"
                                )

                                # Validate with game logic
                                validated_down = self.validate_down_with_game_logic(
                                    "2nd Down", clean_text, frame_number
                                )
                    ...
                    for pattern in third_down_patterns:
                        try:
                            if re.search(pattern, clean_text):
                                print(
                                    f"✅ 3rd DOWN DETECTED! Pattern: {pattern}, Text: '{clean_text}'"
                                )

                                # Validate with game logic
                                validated_down = self.validate_down_with_game_logic(
                                    "3rd Down", clean_text, frame_number
                                )
    ...
    def run(self):
        try:
                        ...
                        if results and len(results) > 0:
                            ...
                            if boxes is not None and len(boxes) > 0:
                                ...
                                if hud_boxes:
                                    ...
                                    first_down_detected, situation = self.detect_down_and_distance(
                                        frame, hud_boxes[0], frame_count
                                    )

                                    # SEPARATE: Check for team scores and possession indicator
                                    scores_detected, score_info = (
                                        self.detect_team_scores_and_possession(
                                            frame, hud_boxes[0], frame_count
                                        )
                                    )
                                    ...
                                    if first_down_detected and situation:
                                        ...
                                        if situation_key and self.situation_preferences.get(
                                            situation_key, False
                                        ):
                                            should_create_clip = True
                                            print(
                                                f"🎯 Situation '{situation}' matches selected preference '{situation_key}'"
                                            )
                                        else:
                                            print(
                                                f"⏭️ Situation '{situation}' not selected for clipping"
                                            )
                                    ...
                                    if should_create_clip:
                                        ...
                                        required_gap = (
                                            180  # Reduced to 180 frames (3 seconds) for quick plays
                                        )

                                        if (
                                            clips_detected == 0
                                            or frames_since_last_clip > required_gap
                                        ):
                                            start_time = (
                                                frame_count / fps
                                            ) - 8.0  # 8 seconds before (increased from 2.5)
                                            end_time = (
                                                frame_count / fps
                                            ) + 4.0  # 4 seconds after (increased from 2.5)
                                            ...
                                            print(
                                                f"⏱️ {situation} detected but too close to last clip ({gap_seconds:.1f}s < {required_seconds:.1f}s required)"
                                            )
            ...
            self.progress_updated.emit(
                100, f"✅ Analysis complete: {clips_detected} clips detected"
            )
    ...
    def check_for_missed_third_down_forward(self, current_frame, detected_down):
        ...
        print(
            f"🔍 FORWARD DETECTION: 1st down detected, checking for missed 3rd down conversion..."
        )
    ...
    def detect_possession_change_scenarios(self, current_frame):
        ...
        if len(self.recent_downs) >= 2:
            ...
            if (
                len(last_two_downs) == 2
                and last_two_downs[0]["down"] == "2nd Down"
                and last_two_downs[1]["down"] == "1st Down"
            ):
                ...
                if frames_between > 180:  # More than 3 seconds apart
                    print(
                        f"    🔍 SUSPICIOUS PATTERN: 2nd → 1st down with {frames_between} frames gap"
                    )
                    ...
                    if any(x in second_text for x in ["3ND", "3D", "RD", "3R", "SD", "3RO"]):
                        print(
                            f"    🎯 CORRECTION: '2nd down' text '{second_text}' contains 3rd down indicators"
                        )
    ...
    def detect_team_scores_and_possession(self, frame, hud_box, frame_number=0):
        ...
        try:
            ...
            print(
                f"🏈 TEAM SCORES REGION: ({score_x_start}-{score_x_end}, {score_y_start}-{score_y_end})"
            )
