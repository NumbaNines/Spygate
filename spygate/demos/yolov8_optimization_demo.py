...
class OptimizationDemo:
    ...
    def _create_test_images(self, count: int = 5) -> List[np.ndarray]:
        ...
        for i in range(count):
            ...
            cv2.putText(image, "HOME 14 - 7 AWAY", (15, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
            
            # Down and distance (bottom)
            cv2.rectangle(image, (500, 680), (780, 710), (255, 255, 0), -1)
            cv2.putText(image, f"3rd & {5 + i}", (510, 700), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
    ...
    def demonstrate_hardware_configurations(self):
        ...
        for tier in [HardwareTier.ULTRA_LOW, HardwareTier.LOW, HardwareTier.MEDIUM, 
    ...
    def demonstrate_adaptive_optimization(self, model: EnhancedYOLOv8):
        ...
        logger.info(f"Max inference time threshold: {model.optimization_config.max_inference_time}s")
        ...
        for i in range(3):
            model.performance_metrics.add_measurement(
                inference_time=0.5,  # Simulated slow inference
                memory_mb=800.0,
                batch_size=1
            )
    ...
    def demonstrate_memory_integration(self, model: EnhancedYOLOv8):
        ...
        for size in buffer_sizes:
            ...
            logger.info(f"  Allocated buffer {size}: {buffer.shape if hasattr(buffer, 'shape') else 'Success'}")
    ...
    def run_complete_demo(self):
        ...
        try:
            ...
            logger.info("="*60)
            logger.info("Priority 3: YOLOv8 Model Configuration Optimization features demonstrated!")
