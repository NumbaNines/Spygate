...
from .yolo11_config import ModelSize, YOLO11Config
...
class HUDTextDetect(Detect):
    ...
    def __init__(self, nc=10, ch=()):
        ...
        self.angle_pred = nn.ModuleList(
            nn.Sequential(
                Conv(x, x, 3, g=4), nn.Conv2d(x, 2, 1), nn.Tanh()  # 2 channels for sin/cos
            )
            for x in ch
        )
        ...
        self.text_conf = nn.ModuleList(
            nn.Sequential(Conv(x, x // 2, 1), nn.Conv2d(x // 2, 1, 1), nn.Sigmoid()) for x in ch
        )
