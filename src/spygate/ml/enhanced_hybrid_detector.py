...
class EnhancedHybridDetector:
    ...
    def _process_region_with_opencv(self, frame: np.ndarray, region: DetectionRegion) -> List[Dict[str, Any]]:
    ...
    def _validate_triangle_geometry(self, triangle) -> bool:
        ...
        angle_valid = all(abs(angle - 60) < self.triangle_params['angle_threshold'] for angle in angles)
        
        # Check aspect ratio
        x, y, w, h = cv2.boundingRect(triangle)
        aspect_ratio = w / h if h != 0 else 0
        ratio_valid = (self.triangle_params['aspect_ratio_range'][0] <= 
                      aspect_ratio <= 
        ...
        for region in regions:
            ...
            for past_regions in self.detection_history:
                for past_region in past_regions:
                    if (past_region.class_name == region.class_name and
                        self._iou(past_region.bbox, region.bbox) > 0.5):
            ...
            if matching_history:
                ...
                smoothed_regions.append(DetectionRegion(
