...
class TestMultiObjectTracker:
    ...
    def test_basic_tracking(self, multi_tracker, complex_scene_sequence):
        ...
        for i, frame in enumerate(complex_scene_sequence[1:], 1):
            ...
            for obj, expected in zip(tracked_objects, expected_positions):
                assert abs(obj.bbox[0] - expected[0]) < 15, f"Object tracking deviation too large at frame {i}"
                assert abs(obj.bbox[1] - expected[1]) < 15, f"Object tracking deviation too large at frame {i}"
    ...
    def test_occlusion_handling(self, multi_tracker, occlusion_sequence):
        ...
        initial_objects = [
            (100, 100, 50, 50),  # Object 1
            (200, 100, 50, 50)   # Object 2
        ]
    ...
    def test_error_handling(self, multi_tracker):
        ...
        # Test invalid bbox
        with pytest.raises(ValueError):
            multi_tracker.add_object(np.zeros(TEST_FRAME_SIZE + (3,), dtype=np.uint8), (-1, -1, 50, 50))
