...
class ContentTypeDetector:
    ...
    def _analyze_content_indicators(self, frame: np.ndarray, hud_region: np.ndarray) -> Dict[str, float]:
    ...
    def _analyze_hud_consistency(self, hud_region: np.ndarray) -> float:
        ...
        gray = cv2.cvtColor(hud_region, cv2.COLOR_BGR2GRAY) if len(hud_region.shape) == 3 else hud_region
    ...
    def _classify_content_type(self, indicators: Dict[str, float]) -> ContentType:
        ...
        logger.debug(f"Content classification - Streamer: {streamer_score:.3f}, Raw: {raw_score:.3f}")
...
class TemplateEngine:
    ...
    def load_templates(self):
        ...
        logger.info(f"Loaded {len(self.raw_templates)} raw templates, {len(self.streamer_templates)} streamer templates")
...
class ContentAwareDownDetector:
    ...
    def _detect_raw_gameplay(self, region: np.ndarray, content_type: ContentType) -> DetectionResult:
    ...
    def _detect_streamer_content(self, region: np.ndarray, content_type: ContentType) -> DetectionResult:
    ...
    def _detect_unknown_content(self, region: np.ndarray, content_type: ContentType) -> DetectionResult:
    ...
    def _detect_with_ocr(self, region: np.ndarray, content_type: ContentType, method: str) -> DetectionResult:
...
def create_sample_templates():
    ...
    common_downs = [
        ("1st_10", 1, 10), ("2nd_10", 2, 10), ("3rd_10", 3, 10), ("4th_10", 4, 10),
        ("1st_5", 1, 5), ("2nd_7", 2, 7), ("3rd_3", 3, 3), ("4th_1", 4, 1),
        ("3rd_goal", 3, 0), ("4th_goal", 4, 0)
    ]
    ...
    for name, down, distance in common_downs:
        ...
        # Add text
        cv2.putText(template, f"{down} & {distance}" if distance > 0 else f"{down} & GOAL", 
