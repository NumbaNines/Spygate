...
class TestTrackingPipeline:
    ...
    def test_pipeline_configuration(self):
        ...
        config = PipelineConfig(
            game_version=GameVersion.MODERN, tracking_mode=TrackingMode.STANDARD
        )
        ...
        full_config = PipelineConfig(
            ...
            preprocessing=PreprocessingConfig(
                enable_noise_reduction=True, enable_stabilization=True
            ),
            tracking=TrackingConfig(enable_gpu=True, enable_prefetch=True),
            formation=FormationConfig(max_players=22, field_zones=4),
        )
    ...
    @pytest.mark.parametrize(
        "tracking_mode",
        [TrackingMode.STANDARD, TrackingMode.HIGH_PERFORMANCE, TrackingMode.QUALITY],
    )
