...
class MemoryEfficientDetect(Detect):
    ...
    def __init__(self, nc=10, ch=()):
        ...
        self.processors = nn.ModuleList([
            self.feature_processor(x) for x in ch
        ])
...
class DeformableConv2d(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, groups: int = 1):
        ...
        self.offset_conv = Conv(in_channels, 2 * kernel_size * kernel_size, kernel_size, groups=groups)
        self.modulation_conv = Conv(in_channels, kernel_size * kernel_size, kernel_size, groups=groups)
        self.regular_conv = Conv(in_channels, out_channels, kernel_size, padding=padding, groups=groups)
...
class CustomYOLO11(DetectionModel):
    ...
    def _optimize_for_low_end(self):
        ...
        self.model[-1] = MemoryEfficientDetect(
            nc=self.model[-1].nc,
            ch=self.model[-1].ch
        )
