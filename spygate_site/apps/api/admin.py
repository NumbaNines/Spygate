...
class APIKeyAdmin(admin.ModelAdmin):
    list_display = ('name', 'user', 'permission_level', 'is_active', 'created_at', 'expires_at', 'is_valid')
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'user', 'permission_level')
        }),
        (_('Settings'), {
            'fields': ('is_active', 'expires_at', 'allowed_ips')
        }),
        (_('Metadata'), {
...
class APIUsageAdmin(admin.ModelAdmin):
    ...
    ordering = ('-timestamp',)
    readonly_fields = ('api_key', 'endpoint', 'method', 'status_code', 'response_time', 
                      'timestamp', 'ip_address', 'user_agent', 'request_data')
...
class WebhookDeliveryAdmin(admin.ModelAdmin):
    list_display = ('webhook', 'event_type', 'success', 'response_status', 'timestamp', 'retry_count')
    list_filter = ('event_type', 'success', 'timestamp')
    search_fields = ('webhook__name', 'event_type')
    ordering = ('-timestamp',)
    readonly_fields = ('webhook', 'event_type', 'payload', 'response_status', 'response_body', 
                      'timestamp', 'success', 'retry_count')
